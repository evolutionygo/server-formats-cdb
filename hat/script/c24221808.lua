...
function c24221808.initial_effect(c)
	--synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunctionEx(Card.IsRace,RACE_PSYCHIC),1,1,aux.NonTunerEx(Card.IsRace,RACE_PSYCHIC),2,99)
	c:EnableReviveLimit()
	--remove
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc24221808(c24221808,0))
	...
	e2:SetDescription(aux.Stringc24221808(c24221808,1))
...
function c24221808.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_REMOVED) then
		if c:IsRelateToEffect(e) then
			...
			if c:GetFlagEffect(c24221808)==0 then
				sg:Clear()
				c:RegisterFlagEffect(c24221808,RESET_EVENT|RESETS_STANDARD&~(RESET_LEAVE|RESET_TEMP_REMOVE|RESET_TOGRAVE|RESET_TURN_SET),0,1)
			end
			sg:AddCard(tc)
...
function c24221808.spcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and e:GetHandler():GetFlagEffect(c24221808)~=0
end
