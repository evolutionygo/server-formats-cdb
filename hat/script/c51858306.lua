...
function c51858306.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc51858306(c51858306,0))
	...
	e2:SetDescription(aux.Stringc51858306(c51858306,1))
...
function c51858306.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		e:SetLabelObject(tc)
		e:GetHandler():RegisterFlagEffect(c51858306,RESET_EVENT+0x1e60000,0,1)
		tc:RegisterFlagEffect(c51858306,RESET_EVENT+RESETS_STANDARD,0,1)
	end
end
function c51858306.thcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsFaceup() and e:GetHandler():GetFlagEffect(c51858306)~=0
end
function c51858306.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=e:GetLabelObject():GetLabelObject()
	if chk==0 then return tc and tc:GetFlagEffect(c51858306)~=0 and tc:IsAbleToHand() end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,tc,1,0,0)
end
function c51858306.thop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject():GetLabelObject()
	if tc:GetFlagEffect(c51858306)~=0 and Duel.SendtoHand(tc,nil,REASON_EFFECT)~=0 then
