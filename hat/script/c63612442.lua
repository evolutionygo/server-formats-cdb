...
function c63612442.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc63612442(c63612442,0))
...
c63612442.listed_series={0x100d}
...
function c63612442.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:GetFlagEffect(c63612442)==0 or c:GetFlagEffect(c63612442+1)==0 end
	local t1=c:GetFlagEffect(c63612442)
	local t2=c:GetFlagEffect(c63612442+1)
	local op=0
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringc63612442(c63612442,0))
	if t1==0 and t2==0 then
		op=Duel.SelectOption(tp,aux.Stringc63612442(c63612442,1),aux.Stringc63612442(c63612442,2))
	elseif t1==0 then op=Duel.SelectOption(tp,aux.Stringc63612442(c63612442,1))
	else Duel.SelectOption(tp,aux.Stringc63612442(c63612442,2)) op=1 end
	e:SetLabel(op)
	if op==0 then c:RegisterFlagEffect(c63612442,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	else c:RegisterFlagEffect(c63612442+1,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1) end
end
function c63612442.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetLabel()==0 then
		...
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc63612442(c63612442,3))
