...
function c67556500.initial_effect(c)
	c:SetSPSummonOnce(c67556500)
	--synchro summon
	aux.AddSynchroProcedure(c,nil,1,1,aux.NonTuner(nil),1,99)
	c:EnableReviveLimit()
	--level
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc67556500(c67556500,0))
...
function c67556500.lvop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFaceup() and c:IsRelateToEffect(e) then
		...
		c:RegisterEffect(e1)
		fc67556500=c:GetRealFieldID()
	end
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_FIELD)
	e2:SetCode(EFFECT_CANNOT_SPECIAL_SUMMON)
	e2:SetProperty(EFFECT_FLAG_PLAYER_TARGET+EFFECT_FLAG_CLIENT_HINT)
	e2:SetDescription(aux.Stringc67556500(c67556500,1))
	e2:SetTargetRange(1,0)
	e2:SetReset(RESET_PHASE+PHASE_END)
	e2:SetTarget(c67556500.splimit)
	e2:SetLabel(fc67556500)
