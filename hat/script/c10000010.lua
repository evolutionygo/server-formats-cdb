...
function c10000010.initial_effect(c)
	...
	e2:SetOperation(function(e) Duel.SetChainLimitTillChainEnd(c10000010.genchainlm(e:GetHandler())) end)
	c:RegisterEffect(e2)
	--One Turn Kill
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc10000010(c10000010,0))
	e3:SetCategory(CATEGORY_ATKCHANGE)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_SUMMON_SUCCESS)
	e3:SetCost(c10000010.atkcost)
	e3:SetOperation(c10000010.atkop)
	c:RegisterEffect(e3)
	--Destroy 1 monster on the field
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc10000010(c10000010,1))
	e4:SetCategory(CATEGORY_DESTROY)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCost(c10000010.descost)
	e4:SetTarget(c10000010.destg)
	e4:SetOperation(c10000010.desop)
	c:RegisterEffect(e4)
end
function c10000010.spval(e,se,sp,st)
