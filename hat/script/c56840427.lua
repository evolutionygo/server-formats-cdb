...
function c56840427.initial_effect(c)
	...
	aux.AddXyzProcedure(c,aux.FilterBoolFunctionEx(Card.IsAttribute,ATTRIBUTE_LIGHT),4,3,c56840427.ovfilter,aux.Stringc56840427(c56840427,0))
	--Make this card gain 500 ATK and 1 monster your opponent controls lose 1000 ATK until the end of that turn
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc56840427(c56840427,1))
	e1:SetCategory(CATEGORY_ATKCHANGE)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(function(e,tp) return Duel.GetLP(tp)<=1000 end)
	e1:SetCost(aux.dxmcostgen(1,1,nil))
	e1:SetTarget(c56840427.atktg)
	e1:SetOperation(c56840427.atkop)
	c:RegisterEffect(e1)
end
c56840427.aux.xyz_number=39
c56840427.listed_names={84013237} --"Number 39: Utopia"
function c56840427.ovfilter(c,tp,lc)
	return c:IsFaceup() and c:IsSummonCode(lc,SUMMON_TYPE_XYZ,tp,84013237)
end
function c56840427.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) end
end
function c56840427.atkop(e,tp,eg,ep,ev,re,r,rp)
