...
function c89397517.initial_effect(c)
	...
	e1:SetCountLimit(1,c89397517,EFFECT_COUNT_CODE_OATH)
	e1:SetCost(c89397517.cost)
	e1:SetTarget(c89397517.target)
	e1:SetOperation(c89397517.activate)
	c:RegisterEffect(e1)
end
c89397517.listed_series={0xa1,0xa0}
function c89397517.cfilter(c,tp)
	return c:IsRace(RACE_WARRIOR) and Duel.GetMZoneCount(tp,c)>0
end
function c89397517.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(1)
	if chk==0 then return Duel.CheckLPCost(tp,2000) and Duel.CheckReleaseGroupCost(tp,c89397517.cfilter,1,false,nil,nil,tp) end
	Duel.PayLPCost(tp,2000)
	local sg=Duel.SelectReleaseGroupCost(tp,c89397517.cfilter,1,1,false,nil,nil,tp)
