...
function c22359980.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if a and a:IsControler(1-tp) and a:IsFaceup() and a:IsLocation(LOCATION_MZONE) then
		...
		if a:GetFlagEffect(c22359980)==0 then
			a:RegisterFlagEffect(c22359980,RESET_EVENT+RESETS_STANDARD,0,1)
		end
	end
end
function c22359980.atktg(e,c)
	return c:GetFlagEffect(c22359980)~=0 and e:GetLabelObject():IsContains(c)
end
...
function c22359980.mtop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.CheckLPCost(tp,2000) and Duel.SelectYesNo(tp,aux.Stringc22359980(c22359980,0)) then
