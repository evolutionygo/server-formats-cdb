...
function c78358521.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc78358521(c78358521,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCountLimit(1,c78358521)
	...
	e2:SetDescription(aux.Stringc78358521(c78358521,1))
...
function c78358521.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() then
		c:RegisterFlagEffect(c78358521,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	end
end
function c78358521.sumcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetFlagEffect(c78358521)~=0
end
...
function c78358521.decop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.GetFlagEffect(tp,c78358521+1)~=0 then return end
	local e1=Effect.CreateEffect(c)
	...
	e2:SetCode(c78358521)
	e2:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e2:SetRange(LOCATION_GRAVE)
	e2:SetTargetRange(1,0)
	e2:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
	c:RegisterEffect(e2)
	Duel.RegisterFlagEffect(tp,c78358521+1,RESET_PHASE+PHASE_END,0,1)
end
function c78358521.econ(e)
	return #{Duel.GetPlayerEffect(e:GetHandlerPlayer(),c78358521)}~=0
end
