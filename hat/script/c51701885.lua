...
function c51701885.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc51701885(c51701885,0))
	...
	e2:SetDescription(aux.Stringc51701885(c51701885,1))
...
function c51701885.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if c:IsRelateToEffect(e) then
			c:RegisterFlagEffect(c51701885,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,3)
			tc:RegisterFlagEffect(c51701885,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,3)
			e:SetLabelObject(tc)
		end
	end
end
function c51701885.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return tc and Duel.GetTurnCount()~=tc:GetTurnID() and Duel.GetTurnPlayer()==tp
		and c:GetFlagEffect(c51701885)~=0 and tc:GetFlagEffect(c51701885)~=0
end
function c51701885.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	e:GetHandler():ResetFlagEffect(c51701885)
