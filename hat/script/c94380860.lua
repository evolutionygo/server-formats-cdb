...
function c94380860.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc94380860(c94380860,0))
...
c94380860.aux.xyz_number=103
function c94380860.filter(c,e)
...
function c94380860.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_MZONE) and c94380860.filter(chkc,e) end
	if chk==0 then
		--retain applicable targets in case cost makes an indirect change to ATK
		local g=Duel.GetMatchingGroup(c94380860.filter,tp,0,LOCATION_MZONE,nil,e)
