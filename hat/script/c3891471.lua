...
function c3891471.initial_effect(c)
	...
	e4:SetDescription(aux.Stringc3891471(c3891471,0))
...
function c3891471.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) then
		...
		if Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP_ATTACK) then
			...
			c:SetCardTarget(tc)
			c:RegisterFlagEffect(c3891471,RESET_EVENT+RESETS_STANDARD_EXC_GRAVE,0,1,lv)
		end
	end
end
...
function c3891471.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetFlagEffect(c3891471)~=0 end
	Duel.SetTargetPlayer(tp)
	local lv=e:GetHandler():GetFlagEffectLabel(c3891471)
