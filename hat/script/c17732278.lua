...
local s, c17732278 = GetID()
function c17732278.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc17732278(c17732278,0))
...
function c17732278.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetFlagEffect(c17732278)==0 end
	e:GetHandler():RegisterFlagEffect(c17732278,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_DAMAGE,0,1)
...
function c17732278.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif tc:IsSpell() then
		if c==Duel.GetAttacker() and not c:IsHasEffect(EFFECT_CANNOT_DIRECT_ATTACK)
			and c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SelectYesNo(tp,aux.Stringc17732278(c17732278,1)) then
			Duel.BreakEffect()
