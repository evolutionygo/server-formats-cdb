...
function c20032555.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc20032555(c20032555,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1,c20032555)
	e1:SetTarget(c20032555.efftg)
	e1:SetOperation(c20032555.effop)
	c:RegisterEffect(e1)
end
c20032555.listed_series={0x1083}
...
function c20032555.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if e:GetHandler():GetLevel()==8 then
		opt=Duel.SelectOption(tp,aux.Stringc20032555(c20032555,1))
	else
		opt=Duel.SelectOption(tp,aux.Stringc20032555(c20032555,1),aux.Stringc20032555(c20032555,2))
	end
	e:SetLabel(opt)
	if opt==0 then
