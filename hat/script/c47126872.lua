...
function c47126872.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc47126872(c47126872,0))
	...
	e2:SetDescription(aux.Stringc47126872(c47126872,1))
...
function c47126872.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		if Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 and e:GetHandler():IsRelateToEffect(e) then
			e:SetLabelObject(tc)
			tc:RegisterFlagEffect(c47126872,RESET_EVENT+RESETS_STANDARD,0,1)
		end
	end
end
function c47126872.setcon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject():GetLabelObject()
	return tc and tc:GetFlagEffect(c47126872)~=0 and not e:GetHandler():IsLocation(LOCATION_DECK)
end
