...
function c96381979.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc96381979(c96381979,0))
	...
	e2:SetDescription(aux.Stringc96381979(c96381979,1))
	...
	e3:SetDescription(aux.Stringc96381979(c96381979,2))
...
function c96381979.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local nc=Duel.IsExistingMatchingCard(c96381979.cfilter,tp,LOCATION_ONFIELD,0,3,nil)
	if chk==0 then return Duel.IsPlayerAffectedByEffect(tp,CARD_FIRE_FIST_EAGLE) or nc end
	if nc and not (Duel.IsPlayerAffectedByEffect(tp,CARD_FIRE_FIST_EAGLE) and Duel.SelectYesNo(tp,aux.Stringc96381979(CARD_FIRE_FIST_EAGLE,0))) then
