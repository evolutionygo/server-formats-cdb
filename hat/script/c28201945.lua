...
function c28201945.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc28201945(c28201945,0))
	e3:SetCategory(CATEGORY_SEARCH+CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_CHAINING)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCountLimit(1,c28201945)
...
c28201945.listed_series={0x4c,0x89,0x108a}
...
function c28201945.filter(c,e,tp,ft)
	return c:IsSetCard(0x108a) and c:GetCode()~=c28201945 and c:IsMonster() and (c:IsAbleToHand() or (ft>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)))
end
...
function c28201945.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		if ft>0 and tc:IsCanBeSpecialSummoned(e,0,tp,false,false)
			and (not tc:IsAbleToHand() or Duel.SelectYesNo(tp,aux.Stringc28201945(c28201945,1))) then
