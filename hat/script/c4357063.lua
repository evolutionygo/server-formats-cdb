...
function c4357063.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc4357063(c4357063,0))
...
c4357063.listed_series={SET_CHRONOMALY}
function c4357063.costfilter(c,e,tp)
	return c:IsSetCard(SET_CHRONOMALY) and c:IsAbleToRemoveAsCost() and c:IsMonster() and c:HasLevel() and aux.SpElimFilter(c,true)
		and Duel.GetMZoneCount(tp,c)>0 and Duel.IsExistingTarget(c4357063.spfilter,tp,LOCATION_GRAVE,0,1,c,e,tp,c:GetLevel())
end
...
function c4357063.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(c4357063.costfilter,tp,LOCATION_MZONE|LOCATION_GRAVE,0,1,nil,e,tp) end
