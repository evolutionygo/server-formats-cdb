...
function c15574615.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc15574615(c15574615,0))
...
c15574615.listed_names={80208158,16796157,43791861,79185500}
function c15574615.chk(c,sg,g,code,...)
	if not c:IsCode(code) then return false end
	local res
	if ... then
		g:AddCard(c)
		res=sg:IsExists(c15574615.chk,1,g,sg,g,...)
		g:RemoveCard(c)
	else
		res=true
	end
...
function c15574615.rescon(sg,e,tp,mg)
	return aux.ChkfMMZ(1)(sg,e,tp,mg) and sg:IsExists(c15574615.chk,1,nil,sg,Group.CreateGroup(),80208158,16796157,43791861,79185500)
end
function c15574615.spcon(e,c)
	...
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-4 and #g1>0 and #g2>0 and #g3>0 and #g4>0
		and aux.SelectUnselectGroup(g,e,tp,4,4,c15574615.rescon,0)
end
function c15574615.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g=aux.SelectUnselectGroup(rg,e,tp,4,4,c15574615.rescon,1,tp,HINTMSG_TOGRAVE,nil,nil,true)
	if #g>0 then
		g:KeepAlive()
		e:SetLabelObject(g)
		return true
...
function c15574615.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)
		...
		and Duel.IsExistingTarget(c15574615.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp,79185500)
	end
