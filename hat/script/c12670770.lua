...
c12670770.listed_names={CARD_CYBER_DRAGON}
...
function c12670770.acttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local c=e:GetHandler()
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc12670770(c12670770,1))
	...
	if Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,CARD_CYBER_DRAGON),tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)
		...
		and Duel.SelectYesNo(tp,aux.Stringc12670770(c12670770,0)) then
		Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,tp,LOCATION_DECK)
		c:RegisterFlagEffect(c12670770,RESETS_STANDARD_PHASE_END,0,1)
		c:RegisterFlagEffect(0,RESET_CHAIN,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringc12670770(c12670770,0))
	end
end
function c12670770.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return e:GetHandler():GetFlagEffect(c12670770)==0
		and Duel.IsExistingMatchingCard(c12670770.filter2,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,tp,LOCATION_DECK)
	e:GetHandler():RegisterFlagEffect(c12670770,RESETS_STANDARD_PHASE_END,0,1)
end
function c12670770.operation(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():GetFlagEffect(c12670770)==0 or not e:GetHandler():IsRelateToEffect(e) then return end
...
function c12670770.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	aux.RegisterClientHint(e:GetHandler(),nil,tp,1,0,aux.Stringc12670770(c12670770,3),nil)
end
