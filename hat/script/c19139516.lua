...
function c19139516.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc19139516(c19139516,0))
	...
	e3:SetDescription(aux.Stringc19139516(c19139516,1))
...
c19139516.listed_series={SET_SCRAP}
c19139516.listed_names={c19139516}
function c19139516.regop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsDefensePos() and c:IsFaceup() then
		c:RegisterFlagEffect(c19139516,RESET_EVENT|RESETS_STANDARD|RESET_PHASE|PHASE_BATTLE,0,1)
	end
end
function c19139516.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetFlagEffect(c19139516)>0 end
...
function c19139516.thfilter(c)
	return c:IsSetCard(SET_SCRAP) and c:IsMonster() and c:IsAbleToHand() and not c:IsCode(c19139516)
end
function c19139516.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c19139516.thfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c19139516.thfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c19139516.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
