...
function c98358303.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc98358303(c98358303,0))
	...
	e2:SetDescription(aux.Stringc98358303(c98358303,1))
...
function c98358303.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if c:IsRelateToEffect(e) then
			c:RegisterFlagEffect(c98358303,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,2)
			tc:RegisterFlagEffect(c98358303,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,2)
			e:SetLabelObject(tc)
		end
	end
end
function c98358303.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return tc and Duel.GetTurnCount()~=tc:GetTurnID()
		and c:GetFlagEffect(c98358303)~=0 and tc:GetFlagEffect(c98358303)~=0
end
