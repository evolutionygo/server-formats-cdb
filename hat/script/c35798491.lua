...
function c35798491.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Roll a six-sc35798491ed die when resolving an opponent's card effect that targets an "Archfiend" monster you control
...
c35798491.listed_series={SET_ARCHFIEND}
c35798491.roll_dice=true
...
function c35798491.filter(c,tp,re)
...
function c35798491.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tg or not tg:IsExists(c35798491.filter,1,nil,tp,re) or not Duel.IsChainDisablable(ev) then return false end
	local rc=re:GetHandler()
	Duel.Hint(HINT_CARD,1-tp,c35798491)
