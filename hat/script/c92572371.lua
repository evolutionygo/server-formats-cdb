...
function c92572371.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc92572371(c92572371,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_GRAVE)
	e1:SetCountLimit(1,c92572371)
	e1:SetCost(c92572371.spcost)
	e1:SetTarget(c92572371.sptg)
	e1:SetOperation(c92572371.spop)
	c:RegisterEffect(e1)
end
c92572371.listed_series={0x79,0x7c}
...
function c92572371.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local nc=#sg>=2 and (ft>0 or (ct<3 and sg:IsExists(c92572371.mzfilter,ct,nil)))
	if chk==0 then 
		if Duel.IsPlayerAffectedByEffect(tp,CARD_FIRE_FIST_EAGLE) then 
			return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 
		else return nc end
	end
	if nc and not (Duel.IsPlayerAffectedByEffect(tp,CARD_FIRE_FIST_EAGLE) and Duel.SelectYesNo(tp,aux.Stringc92572371(CARD_FIRE_FIST_EAGLE,0))) then
		local g=nil
		if ft<=0 then
