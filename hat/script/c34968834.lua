...
function c34968834.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc34968834(c34968834,0))
...
function c34968834.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if rp~=tp and tp==e:GetLabel() and Duel.IsExistingMatchingCard(c34968834.filter,tp,LOCATION_DECK,0,1,nil,e,tp)
		and Duel.SelectYesNo(tp,aux.Stringc34968834(c34968834,1)) then
		Duel.BreakEffect()
		...
		if tc then
			...
			local op=0
			if b1 and b2 then
				op=Duel.SelectOption(tp,aux.Stringc34968834(c34968834,2),aux.Stringc34968834(c34968834,3))
			elseif b1 then
				op=Duel.SelectOption(tp,aux.Stringc34968834(c34968834,2))
			elseif b2 then
				op=Duel.SelectOption(tp,aux.Stringc34968834(c34968834,3))+1
			else return end
			if op==0 then
