--究極封印神エクゾディオス
--Exodius the Ultimate Forbc13893596den Lord
function c13893596.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc13893596(c13893596,0))
...
c13893596.listed_series={SET_FORBIDDEN_ONE}
...
function c13893596.winfilter(c,rc)
...
function c13893596.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SendtoGrave(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) and c:IsRelateToEffect(e) then
		...
		--Win condition
		if not (c:IsFaceup() and c:IsOriginalCode(c13893596) and c:IsControler(tp)) then return end
		local g=Duel.GetMatchingGroup(c13893596.winfilter,tp,LOCATION_GRAVE,0,nil,c)
		if c13893596.winfilter(tc,c) and g:GetClassCount(Card.GetCode)==5 then
