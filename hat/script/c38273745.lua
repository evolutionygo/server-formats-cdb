...
function c38273745.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc38273745(c38273745,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1)
	e1:SetCost(aux.dxmcostgen(1,1,nil))
	e1:SetTarget(c38273745.efftg)
	e1:SetOperation(c38273745.effop)
	c:RegisterEffect(e1,false,REGISTER_FLAG_DETACH_XMAT)
end
function c38273745.efftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		local op=e:GetLabel()
	...
	local b1=not c:HasFlagEffect(c38273745)
	...
	local b2=not c:HasFlagEffect(c38273745+1)
		and Duel.GetFieldGroupCount(1-tp,LOCATION_HAND,0)>0
	--Banish 1 card from your opponent's GY
	local b3=not c:HasFlagEffect(c38273745+2)
		and Duel.IsExistingTarget(Card.IsAbleToRemove,tp,0,LOCATION_GRAVE,1,nil)
	if chk==0 then return b1 or b2 or b3 end
	local op=Duel.SelectEffect(tp,
		{b1,aux.Stringc38273745(c38273745,1)},
		{b2,aux.Stringc38273745(c38273745,2)},
		{b3,aux.Stringc38273745(c38273745,3)})
	e:SetLabel(op)
	c:RegisterFlagEffect(c38273745+op-1,RESET_EVENT|RESETS_STANDARD,0,1)
	if op==1 then
...
function c38273745.effop(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
		--Return 1 card your opponent controls to the hand
