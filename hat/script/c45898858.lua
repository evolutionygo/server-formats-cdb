...
function c45898858.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc45898858(c45898858,0))
...
c45898858.listed_names={58071123,22587018,85066822} --Oxyggedon, Hydrogeddon, Water Dragon
...
function c45898858.spfilter(c,e,tp)
...
function c45898858.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.IsExistingMatchingCard(c45898858.spfilter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_GRAVE,0,1,nil,e,tp)
	end
...
function c45898858.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c45898858.spfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_GRAVE,0,1,1,nil,e,tp)
	if #g>0 then
