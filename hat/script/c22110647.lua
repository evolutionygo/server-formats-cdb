...
function c22110647.initial_effect(c)
	...
	e1:SetCondition(c22110647.indcon)
	e1:SetValue(1)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e3:SetDescription(aux.Stringc22110647(c22110647,0))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_TOKEN)
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCountLimit(1)
	e3:SetCost(aux.dxmcostgen(1,1,nil))
	e3:SetTarget(c22110647.sptg)
	e3:SetOperation(c22110647.spop)
	c:RegisterEffect(e3,false,REGISTER_FLAG_DETACH_XMAT)
	--Destroy 1 card on the field
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc22110647(c22110647,1))
	e4:SetCategory(CATEGORY_DESTROY)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_MZONE)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetCountLimit(1)
	e4:SetCost(c22110647.descost)
	e4:SetTarget(c22110647.destg)
	e4:SetOperation(c22110647.desop)
	c:RegisterEffect(e4)
end
c22110647.listed_names={TOKEN_MECHA_PHANTOM_BEAST}
c22110647.listed_series={SET_MECHA_PHANTOM_BEAST}
function c22110647.tknfilter(c)
	return c:IsType(TYPE_TOKEN) or c:IsOriginalType(TYPE_TOKEN)
end
function c22110647.indcon(e)
	return Duel.IsExistingMatchingCard(c22110647.tknfilter,e:GetHandlerPlayer(),LOCATION_ONFIELD,0,1,nil)
end
