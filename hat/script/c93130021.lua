...
function c93130021.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc93130021(c93130021,0))
...
c93130021.listed_names={TOKEN_OPTION}
...
function c93130021.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local t2=Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,c93130021+1,0,TYPES_TOKEN,c:GetAttack(),c:GetDefense(),c:GetLevel(),c:GetRace(),c:GetAttribute())
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringc93130021(c93130021,0))
	if t1 and t2 then
		op=Duel.SelectOption(tp,aux.Stringc93130021(c93130021,1),aux.Stringc93130021(c93130021,2),aux.Stringc93130021(c93130021,3))
	elseif t1 then
		op=Duel.SelectOption(tp,aux.Stringc93130021(c93130021,1),aux.Stringc93130021(c93130021,2))
	elseif t2 then
		op=Duel.SelectOption(tp,aux.Stringc93130021(c93130021,1),aux.Stringc93130021(c93130021,3))
		if op==1 then op=2 end
	else op=Duel.SelectOption(tp,aux.Stringc93130021(c93130021,1)) end
	e:SetLabel(op)
	if op==1 then
...
function c93130021.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif e:GetLabel()==2 then
		...
		if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not c:IsRelateToEffect(e) or c:IsFacedown()
			or not Duel.IsPlayerCanSpecialSummonMonster(tp,c93130021+1,0,TYPES_TOKEN,atk,def,lv,race,att) then return end
		local token=Duel.CreateToken(tp,c93130021+1)
