...
function c84117021.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingTarget(aux.FaceupFilter(Card.IsRace,RACE_SPELLCASTER),tp,LOCATION_MZONE,0,1,nil) then
		opt=Duel.SelectOption(tp,aux.Stringc84117021(c84117021,0),aux.Stringc84117021(c84117021,1))
	end
...
function c84117021.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if opt==0 then
		...
		e7:SetOperation(c84117021.cedop)
...
c84117021.filter=aux.FaceupFilter(Card.IsRace,RACE_SPELLCASTER)
...
function c84117021.sumsuc(e,tp,eg,ep,ev,re,r,rp)
	Duel.SetChainLimitTillChainEnd(c84117021.chlimit)
end
function c84117021.cedop(e,tp,eg,ep,ev,re,r,rp)
	local _,g=Duel.CheckEvent(EVENT_SPSUMMON_SUCCESS,true)
	if g and g:IsExists(c84117021.filter,1,nil) and Duel.CheckEvent(EVENT_SPSUMMON_SUCCESS) then
		Duel.SetChainLimitTillChainEnd(c84117021.chlimit)
	end
end
function c84117021.chlimit(re,rp,tp)
	return rp==tp
end
