...
function c23536866.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc23536866(c23536866,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_GRAVE)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCountLimit(1,c23536866)
	e1:SetCost(aux.bfgcost)
	e1:SetTarget(c23536866.target)
	e1:SetOperation(c23536866.operation)
	c:RegisterEffect(e1)
end
c23536866.listed_names={c23536866}
function c23536866.filter(c,e,tp)
	return c:GetLevel()==5 and c:IsAttribute(ATTRIBUTE_WATER) and not c:IsCode(c23536866)
		and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c23536866.xyzfilter(c,tp,mg)
...
function c23536866.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local exg=Duel.GetMatchingGroup(c23536866.xyzfilter,tp,LOCATION_EXTRA,0,nil,tp,mg)
...
function c23536866.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.BreakEffect()
	local xyzg=Duel.GetMatchingGroup(c23536866.xyzfilter,tp,LOCATION_EXTRA,0,nil,tp,g)
	if #xyzg>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
