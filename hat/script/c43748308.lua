...
function c43748308.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc43748308(c43748308,0))
	...
	e3:SetDescription(aux.Stringc43748308(c43748308,1))
...
c43748308.listed_series={0x7c,0x79}
c43748308.listed_names={c43748308}
...
function c43748308.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if nc and not (Duel.IsPlayerAffectedByEffect(tp,CARD_FIRE_FIST_EAGLE) and Duel.SelectYesNo(tp,aux.Stringc43748308(CARD_FIRE_FIST_EAGLE,0))) then
		local sg=aux.SelectUnselectGroup(g,e,tp,2,2,aux.ChkfMMZ(1),1,tp,HINTMSG_TOGRAVE)
		Duel.SendtoGrave(sg,REASON_COST)
	end
end
function c43748308.spfilter(c,e,tp)
	return c:IsSetCard(0x79) and not c:IsCode(c43748308) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
