...
function c20822520.initial_effect(c)
	...
	e1:SetCountLimit(1,c20822520,EFFECT_COUNT_CODE_OATH)
...
c20822520.listed_series={0x106e}
function c20822520.checkop(e,tp,eg,ep,ev,re,r,rp)
	if re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_SPELL) and not re:GetHandler():IsSetCard(0x106e) then
		Duel.RegisterFlagEffect(rp,c20822520+1,RESET_PHASE+PHASE_END,0,1)
	end
end
...
function c20822520.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,c20822520+1)==0 end
	--oath effects
	...
	aux.RegisterClientHint(e:GetHandler(),nil,tp,1,0,aux.Stringc20822520(c20822520,1),nil)
end
