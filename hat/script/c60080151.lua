...
function c60080151.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:CanAttack() and tc:IsRelateToBattle() and not tc:IsStatus(STATUS_ATTACK_CANCELED) then
		...
		if Duel.Damage(tp,dam,REASON_EFFECT)>0 and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_REMOVED) then
			...
			tc:RegisterFlagEffect(c60080151,RESET_EVENT+RESETS_STANDARD,0,0)
		end
	end
end
function c60080151.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetTurnCount()~=e:GetLabel() and Duel.GetTurnPlayer()~=tp
		and tc:GetFlagEffect(c60080151)~=0 and tc:GetReasonEffect():GetHandler()==e:GetHandler()
end
function c60080151.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,c60080151)
