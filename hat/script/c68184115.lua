...
function c68184115.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc68184115(c68184115,0))
	...
	e3:SetDescription(aux.Stringc68184115(c68184115,1))
...
function c68184115.spfilter(c,e,tp)
	return c:IsSetCard(SET_INZEKTOR) and c:GetCode()~=c68184115 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c68184115.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsRelateToEffect(e) and c:IsFaceup() and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c68184115.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
