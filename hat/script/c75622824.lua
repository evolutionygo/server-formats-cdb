...
c75622824.listed_names={14778250,75622825}
...
function c75622824.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		return ct>0 and (ct==1 or not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)) 
			and Duel.IsPlayerCanSpecialSummonMonster(tp,c75622824+1,0,TYPES_TOKEN,2000,1200,5,RACE_SPELLCASTER,ATTRIBUTE_WIND)
	end
...
function c75622824.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,c75622824+1,0,TYPES_TOKEN,2000,1200,5,RACE_SPELLCASTER,ATTRIBUTE_WIND) then return end
	for i=1,ct do
		local token=Duel.CreateToken(tp,c75622824+1)
