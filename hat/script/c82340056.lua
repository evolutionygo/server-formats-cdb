...
function c82340056.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc82340056(c82340056,0))
...
c82340056.listed_names={82340057} --Cerimonial Token
c82340056.listed_series={SET_EARTHBOUND_IMMORTAL}
...
function c82340056.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) and Duel.GetLocationCount(tp,LOCATION_MZONE)>1
		and Duel.IsPlayerCanSpecialSummonMonster(tp,c82340056+1,0,TYPES_TOKEN,0,0,1,RACE_ROCK,ATTRIBUTE_EARTH)
...
function c82340056.activate(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.NegateAttack() or Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) or Duel.GetLocationCount(tp,LOCATION_MZONE)<2
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,c82340056+1,0,TYPES_TOKEN,0,0,1,RACE_ROCK,ATTRIBUTE_EARTH) then return end
	...
	Duel.ShuffleHand(tp)
	for i=1,2 do
		local token=Duel.CreateToken(tp,c82340056+1)
