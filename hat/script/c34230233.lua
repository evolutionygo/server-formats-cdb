...
function c34230233.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc34230233(c34230233,0))
...
c34230233.listed_series={0x6}
c34230233.listed_names={c34230233}
function c34230233.spfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x6) and not c:IsCode(c34230233) and c:IsAbleToHandAsCost()
end
function c34230233.spcon(e,c)
	if c==nil then return true end
	local tp=e:GetHandlerPlayer()
	local rg=Duel.GetMatchingGroup(c34230233.spfilter,tp,LOCATION_MZONE,0,nil)
...
function c34230233.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)~=0 and rp==1-tp and tp==e:GetLabel() then
		...
		if #hg>0 then
			...
			if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and cc:IsCanBeSpecialSummoned(e,0,tp,false,false)
				and Duel.SelectYesNo(tp,aux.Stringc34230233(c34230233,1)) then
