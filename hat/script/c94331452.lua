...
function c94331452.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc94331452(c94331452,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1)
	e1:SetTarget(c94331452.target(LOCATION_MZONE))
	e1:SetOperation(c94331452.operation)
	c:RegisterEffect(e1)
	--Change the Attribute of 1 "Symphonic Warrior" monster you control
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc94331452(c94331452,0))
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetRange(LOCATION_GRAVE)
	e2:SetCost(aux.bfgcost)
	e2:SetTarget(c94331452.target(0))
	e2:SetOperation(c94331452.operation)
	c:RegisterEffect(e2)
end
c94331452.listed_series={SET_SYMPHONIC_WARRIOR}
function c94331452.target(oppo)
