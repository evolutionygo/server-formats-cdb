...
function c96682430.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc96682430(c96682430,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_DELAY)
	e1:SetCode(EVENT_TO_GRAVE)
	e1:SetCountLimit(1,c96682430)
	e1:SetTarget(c96682430.target)
	e1:SetOperation(c96682430.operation)
	c:RegisterEffect(e1)
end
c96682430.listed_series={0x74}
function c96682430.filter(c,e,tp)
	return c:IsSetCard(0x74) and c:GetCode()~=c96682430 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
