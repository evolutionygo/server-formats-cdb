...
function c21249921.initial_effect(c)
	--synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunctionEx(Card.IsRace,RACE_DRAGON),1,1,aux.NonTunerEx(Card.IsRace,RACE_WINGEDBEAST),1,99)
	c:EnableReviveLimit()
	--equip
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc21249921(c21249921,0))
	...
	c:RegisterEffect(e1)
	aux.AddEREquipLimit(c,nil,c21249921.eqval,Card.EquipByEffectAndLimitRegister,e1)
	--atk
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc21249921(c21249921,1))
...
function c21249921.filter(c)
	return c:IsLevelBelow(3) and c:IsSetCard(0x29) and c:IsRace(RACE_DRAGON) and not c:IsForbc21249921den()
end
...
function c21249921.cfilter(c,tp)
	return c:IsAbleToGraveAsCost() and c:IsControler(tp)
end
function c21249921.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:GetEquipGroup():IsExists(c21249921.cfilter,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=c:GetEquipGroup():FilterSelect(tp,c21249921.cfilter,1,1,nil,tp)
