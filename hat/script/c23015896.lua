...
function c23015896.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc23015896(c23015896,0))
	...
	e3:SetDescription(aux.Stringc23015896(c23015896,1))
	...
	e4:SetDescription(aux.Stringc23015896(c23015896,2))
...
c23015896.listed_series={0x81}
function c23015896.spreg(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if (r&0x41)~=0x41 then return end
	if Duel.GetCurrentPhase()==PHASE_STANDBY then
		e:SetLabel(Duel.GetTurnCount())
		c:RegisterFlagEffect(c23015896,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY,0,2)
	else
		e:SetLabel(0)
		c:RegisterFlagEffect(c23015896,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY,0,1)
	end
end
function c23015896.spcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetLabelObject():GetLabel()~=Duel.GetTurnCount() and e:GetHandler():GetFlagEffect(c23015896)>0
end
function c23015896.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local c=e:GetHandler()
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
	c:ResetFlagEffect(c23015896)
end
...
function c23015896.spfilter(c,e,tp)
	return c:IsSetCard(0x81) and c:GetCode()~=c23015896 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
