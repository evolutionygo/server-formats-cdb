...
function c97489701.initial_effect(c)
	...
	aux.AddSynchroProcedure(c,nil,2,2,aux.FilterBoolFunction(Card.IsCode,CARD_RED_DRAGON_ARCHFIEND),1,1)
	...
	e3:SetDescription(aux.Stringc97489701(c97489701,0))
	e3:SetCategory(CATEGORY_REMOVE)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e3:SetCode(EVENT_ATTACK_ANNOUNCE)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c97489701.nacon)
	e3:SetTarget(c97489701.natg)
	e3:SetOperation(c97489701.naop)
	c:RegisterEffect(e3)
	--Special Summon itself during the End Phase
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc97489701(c97489701,1))
	e4:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e4:SetCode(EVENT_PHASE+PHASE_END)
	e4:SetRange(LOCATION_REMOVED)
	e4:SetCountLimit(1)
	e4:SetCondition(function(e) return e:GetHandler():HasFlagEffect(c97489701) end)
	e4:SetTarget(c97489701.sptg)
	e4:SetOperation(c97489701.spop)
	c:RegisterEffect(e4)
	--Multiple Tuners
...
c97489701.material={CARD_RED_DRAGON_ARCHFIEND}
c97489701.listed_names={CARD_RED_DRAGON_ARCHFIEND}
c97489701.synchro_nt_required=1
...
function c97489701.naop(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if c:IsRelateToEffect(e) and Duel.Remove(c,POS_FACEUP,REASON_EFFECT)~=0 then
		c:RegisterFlagEffect(c97489701,RESETS_STANDARD_PHASE_END,0,1)
	end
end
