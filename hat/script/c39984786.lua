...
function c39984786.valcheck(e,c)
	...
	for tc in aux.Next(g) do
		if tc:IsRace(RACE_REPTILE) and not tc:IsType(TYPE_TOKEN) then
			tc:RegisterFlagEffect(c39984786,RESET_EVENT+RESETS_STANDARD_EXC_GRAVE+RESET_PHASE+PHASE_END,0,1)
			sp=true
		end
	end
	if sp then
		--spsummon
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc39984786(c39984786,0))
...
function c39984786.filter(c,e,tp)
	return c:GetFlagEffect(c39984786)~=0 and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
