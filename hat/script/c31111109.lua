...
function c31111109.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCode2Rep(c,true,true,c31111109.ffilter,2,2,aux.FilterBoolFunctionEx(Card.IsSetCard,0x9),aux.FilterBoolFunctionEx(Card.IsSetCard,0x1f),aux.FilterBoolFunctionEx(Card.IsSetCard,0x8))
	--copy
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc31111109(c31111109,0))
...
c31111109.listed_series={0x9,0x1f,0x8}
c31111109.material_setcode={0x8,0x9,0x1f}
function c31111109.ffilter(c,fc,sumtype,tp)
...
function c31111109.filter(c)
	return (c:IsSetCard(0x9) or c:IsSetCard(0x1f) or c:IsSetCard(0x8)) and c:IsMonster()
		and not c:IsForbc31111109den() and c:IsAbleToRemove() and aux.SpElimFilter(c,true)
end
...
function c31111109.copyop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() and tc and tc:IsRelateToEffect(e) then
		...
		local cc31111109=c:CopyEffect(code,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,1)
		...
		c:RegisterEffect(e1)
		local e2=Effect.CreateEffect(c)
		e2:SetDescription(aux.Stringc31111109(c31111109,1))
...
function c31111109.rstop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local cc31111109=e:GetLabel()
	c:ResetEffect(cc31111109,RESET_COPY)
