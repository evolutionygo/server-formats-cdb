...
function c92729410.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc92729410(c92729410,0))
	...
	c:RegisterEffect(e2)
	Duel.AddCustomActivityCounter(c92729410,ACTIVITY_SPSUMMON,c92729410.counterfilter)
end
function c92729410.counterfilter(c)
	return c:IsRace(RACE_BEAST)
end
function c92729410.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetCustomActivityCount(c92729410,tp,ACTIVITY_SPSUMMON)==0 end
	...
	e2:SetDescription(aux.Stringc92729410(c92729410,1))
...
function c92729410.filter(c,e,tp)
	return c:GetCode()~=c92729410 and c:GetLevel()==2 and c:IsRace(RACE_BEAST)
