...
function c87257460.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc87257460(c87257460,0))
	e1:SetCategory(CATEGORY_EQUIP)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1,0,EFFECT_COUNT_CODE_SINGLE)
	e1:SetCondition(c87257460.eqconignition)
	e1:SetTarget(c87257460.eqtg)
	e1:SetOperation(c87257460.eqop)
	c:RegisterEffect(e1)
	aux.AddEREquipLimit(c,c87257460.eqconignition,c87257460.eqval,c87257460.equipop,e1)
	...
	e2:SetCondition(c87257460.eqconquick)
	c:RegisterEffect(e2)
	aux.AddEREquipLimit(c,c87257460.eqconquick,c87257460.eqval,c87257460.equipop,e2)
	--Special Summon 1 "Allure Queen LV5" from your hand or Deck
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc87257460(c87257460,1))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_PHASE+PHASE_STANDBY)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCountLimit(1)
	e3:SetCondition(function(e,tp) return Duel.IsTurnPlayer(tp) and e:GetHandler():GetEquipGroup():IsExists(Card.HasFlagEffect,1,nil,c87257460) end)
...
c87257460.listed_names={23756165} --"Allure Queen LV5"
c87257460.LVnum=3
c87257460.LVset=SET_ALLURE_QUEEN
function c87257460.eqconignition(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return not c:GetEquipGroup():IsExists(Card.HasFlagEffect,1,nil,c87257460) and not (c:IsOriginalSetCard(SET_ALLURE_QUEEN) and Duel.IsPlayerAffectedByEffect(tp,EFFECT_GOLDEN_ALLURE_QUEEN))
end
function c87257460.eqconquick(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return not c:GetEquipGroup():IsExists(Card.HasFlagEffect,1,nil,c87257460) and c:IsOriginalSetCard(SET_ALLURE_QUEEN) and Duel.IsPlayerAffectedByEffect(tp,EFFECT_GOLDEN_ALLURE_QUEEN)
end
...
function c87257460.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c87257460.eqfilter(chkc) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c87257460.eqfilter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,c87257460.eqfilter,tp,0,LOCATION_MZONE,1,1,nil)
...
function c87257460.equipop(c,e,tp,tc)
	if not c:EquipByEffectAndLimitRegister(e,tp,tc,c87257460) then return end
