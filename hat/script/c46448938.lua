...
function c46448938.initial_effect(c)
	...
	e1:SetCountLimit(1,c46448938,EFFECT_COUNT_CODE_OATH)
	e1:SetTarget(c46448938.target)
	e1:SetOperation(c46448938.activate)
	c:RegisterEffect(e1)
end
c46448938.listed_series={0x106e}
...
function c46448938.sfilter(c)
	return c:IsSetCard(0x106e) and c:GetCode()~=c46448938 and c:IsSpell() and c:IsAbleToHand()
end
...
function c46448938.effop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,c46448938)
	...
	if #g>0 then
		...
		if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsExistingMatchingCard(c46448938.spfilter,tp,LOCATION_DECK,0,1,nil,#g,e,tp)
			and Duel.SelectYesNo(tp,aux.Stringc46448938(c46448938,1)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
			local sg=Duel.SelectMatchingCard(tp,c46448938.spfilter,tp,LOCATION_DECK,0,1,1,nil,#g,e,tp)
