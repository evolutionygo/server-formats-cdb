...
function c7841112.initial_effect(c)
	...
	Synchro.AddMajesticProcedure(c,aux.FilterBoolFunction(Card.IsCode,21159309),true,aux.FilterBoolFunction(Card.IsCode,CARD_STARDUST_DRAGON),true,aux.NonTuner(nil),false)
	--Negate the activation of an opponent's card or effect
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc7841112(c7841112,0))
	e1:SetCategory(CATEGORY_NEGATE+CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetCode(EVENT_CHAINING)
	e1:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(c7841112.negcon)
	e1:SetCost(aux.SelfTributeCost)
	e1:SetTarget(c7841112.negtg)
	e1:SetOperation(c7841112.negop)
	c:RegisterEffect(e1)
	--Negate the effects of a monster the opponent controls
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc7841112(c7841112,1))
	e2:SetCategory(CATEGORY_DISABLE)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1)
	e2:SetTarget(c7841112.distg)
	e2:SetOperation(c7841112.disop)
	c:RegisterEffect(e2)
	--activate limit
	...
	e3:SetOperation(c7841112.alop)
	c:RegisterEffect(e3)
	--Return itself to the Extra Deck and Special Summon 1 "Stardust Dragon"
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc7841112(c7841112,2))
	...
	e4:SetCountLimit(1)
	e4:SetTarget(c7841112.sptg)
	e4:SetOperation(c7841112.spop)
	c:RegisterEffect(e4)
	--Multiple tuners
	local e5=Effect.CreateEffect(c)
	e5:SetType(EFFECT_TYPE_SINGLE)
	e5:SetCode(EFFECT_MATERIAL_CHECK)
	e5:SetValue(c7841112.valcheck)
	c:RegisterEffect(e5)
end
c7841112.material={21159309,CARD_STARDUST_DRAGON}
c7841112.listed_names={21159309,CARD_STARDUST_DRAGON} --Majestic Dragon
c7841112.synchro_nt_required=1
