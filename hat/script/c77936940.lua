...
function c77936940.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc77936940(c77936940,0))
...
function c77936940.reg(e,tp,eg,ep,ev,re,r,rp)
	...
	if (r&0x41)==0x41 and rp==1-tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD)
		...
		c:RegisterFlagEffect(c77936940,RESET_EVENT+RESETS_STANDARD,0,1)
	end
end
function c77936940.spcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:GetTurnID()~=Duel.GetTurnCount() and tp==Duel.GetTurnPlayer() and c:GetFlagEffect(c77936940)>0
end
function c77936940.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local c=e:GetHandler()
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
	c:ResetFlagEffect(c77936940)
end
