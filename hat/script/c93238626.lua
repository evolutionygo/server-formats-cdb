...
function c93238626.initial_effect(c)
	c:SetUniqueOnField(1,0,c93238626)
	--activate
	...
	e2:SetDescription(aux.Stringc93238626(c93238626,0))
	...
	e3:SetDescription(aux.Stringc93238626(c93238626,1))
...
c93238626.listed_series={0x48,0x1048}
function c93238626.checkop(e,tp,eg,ep,ev,re,r,rp)
	Duel.RegisterFlagEffect(ep,c93238626,RESET_PHASE+PHASE_END,0,1)
end
...
function c93238626.rectg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local ct=Duel.GetFlagEffect(tp,c93238626)
...
function c93238626.filter(c,e,tp,mc,no,pg)
	return c.aux.xyz_number==no and c:IsSetCard(0x1048) and mc:IsCanBeXyzMaterial(c,tp)
...
function c93238626.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return #pg<=1 and m and m.aux.xyz_number and Duel.IsExistingMatchingCard(c93238626.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,at,m.aux.xyz_number,pg)
	end
...
function c93238626.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c93238626.filter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,m.aux.xyz_number,pg)
	local sc=g:GetFirst()
	if sc then
		sc:SetMaterial(tc)
		Duel.Overlay(sc,tc)
