--エヴォルド・エルギネル
--Evoltile Elginero
function c91903221.initial_effect(c)
	--Draw 1 card, shuffle 1 Dinousaur monster into the Deck, and search 1 "Evoltile" monster
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc91903221(c91903221,0))
	e1:SetCategory(CATEGORY_DRAW+CATEGORY_TODECK+CATEGORY_SEARCH+CATEGORY_TOHAND)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e1:SetCode(EVENT_RELEASE)
	e1:SetCondition(c91903221.condition)
	e1:SetTarget(c91903221.target)
	e1:SetOperation(c91903221.operation)
	c:RegisterEffect(e1)
end
c91903221.listed_series={0x304e}
function c91903221.condition(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsLocation(LOCATION_GRAVE)
end
function c91903221.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
	Duel.SetPossibleOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_HAND)
	Duel.SetPossibleOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c91903221.tdfilter(c)
	return c:IsRace(RACE_DINOSAUR) and c:IsAbleToDeck()
end
function c91903221.thfilter(c)
	return c:IsSetCard(0x304e) and c:IsMonster() and c:IsAbleToHand()
end
function c91903221.operation(e,tp,eg,ep,ev,re,r,rp)
	if Duel.Draw(tp,1,REASON_EFFECT)==0 then return end
	local g1=Duel.GetMatchingGroup(c91903221.tdfilter,tp,LOCATION_HAND,0,nil)
	local g2=Duel.GetMatchingGroup(c91903221.thfilter,tp,LOCATION_DECK,0,nil)
	if #g1>0 and #g2>0 and Duel.SelectYesNo(tp,aux.Stringc91903221(c91903221,1)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
		local tg1=g1:Select(tp,1,1,nil)
		Duel.ConfirmCards(1-tp,tg1)
		Duel.BreakEffect()
		if Duel.SendtoDeck(tg1,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)==0 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local tg2=g2:Select(tp,1,1,nil)
		Duel.SendtoHand(tg2,nil,REASON_EFFECT)
		Duel.ConfirmCards(1-tp,tg2)
	end
end