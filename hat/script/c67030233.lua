...
function c67030233.initial_effect(c)
	...
	Synchro.AddMajesticProcedure(c,aux.FilterBoolFunction(Card.IsCode,21159309),true,aux.FilterBoolFunction(Card.IsCode,CARD_RED_DRAGON_ARCHFIEND),true,aux.NonTuner(nil),false)
	--Destroy all Defense Position monsters
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc67030233(c67030233,0))
	e1:SetCategory(CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e1:SetCode(EVENT_BATTLED)
	e1:SetCondition(c67030233.descon)
	e1:SetTarget(c67030233.destg)
	e1:SetOperation(c67030233.desop)
	c:RegisterEffect(e1)
	--Negate the effects of 1 monster your opponent controls
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc67030233(c67030233,1))
	e2:SetCategory(CATEGORY_DISABLE+CATEGORY_ATKCHANGE)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1)
	e2:SetTarget(c67030233.distg)
	e2:SetOperation(c67030233.disop)
	c:RegisterEffect(e2)
	--Return itself to the Extra Deck and Special Summon 1 "Red Dragon Archfiend"
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc67030233(c67030233,2))
	...
	e3:SetCountLimit(1)
	e3:SetTarget(c67030233.sptg)
	e3:SetOperation(c67030233.spop)
	c:RegisterEffect(e3)
	...
	e5:SetValue(c67030233.valcheck)
	c:RegisterEffect(e5)
end
c67030233.material={CARD_RED_DRAGON_ARCHFIEND,21159309}
c67030233.listed_names={CARD_RED_DRAGON_ARCHFIEND,21159309} --Majestic Dragon
c67030233.synchro_nt_required=1
