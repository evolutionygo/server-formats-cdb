...
function c81426505.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local t={}
	local p=1
	if Duel.IsExistingMatchingCard(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) then t[p]=aux.Stringc81426505(c81426505,0) p=p+1 end
	if Duel.IsExistingMatchingCard(c81426505.filter2,tp,0,LOCATION_ONFIELD,1,nil) then t[p]=aux.Stringc81426505(c81426505,1) p=p+1 end
	if Duel.IsExistingMatchingCard(Card.IsFacedown,tp,0,LOCATION_SZONE,1,nil) then t[p]=aux.Stringc81426505(c81426505,2) p=p+1 end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringc81426505(c81426505,3))
	local sel=Duel.SelectOption(tp,table.unpack(t))+1
	local opt=t[sel]-aux.Stringc81426505(c81426505,0)
	local sg=nil
