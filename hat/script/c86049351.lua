...
c86049351.listed_series={0x84}
function c86049351.cfilter(c,pos)
...
function c86049351.condition(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.IsExistingMatchingCard(c86049351.cfilter,tp,LOCATION_MZONE,0,1,bc1,pos)
end
...
function c86049351.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsControler(tp) and Duel.SendtoGrave(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) then
		...
		local sc=Duel.SelectMatchingCard(tp,c86049351.cfilter,tp,LOCATION_MZONE,0,1,1,nil,pos):GetFirst()
