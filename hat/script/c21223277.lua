...
function c21223277.initial_effect(c)
	...
	aux.AddXyzProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_NOBLE_KNIGHT),4,2)
	--Equip to itself up to 3 "Noble Arms" Equip Spell Cards from the GY
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc21223277(c21223277,0))
	...
	e2:SetDescription(aux.Stringc21223277(c21223277,1))
...
c21223277.listed_series={SET_NOBLE_ARMS,SET_NOBLE_KNIGHT}
...
function c21223277.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.GetMatchingGroup(c21223277.filter,tp,LOCATION_GRAVE,0,nil,e,tp,c)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c21223277.filter(chkc,e,tp,c) end
	local ft=math.min(Duel.GetLocationCount(tp,LOCATION_SZONE),3)
	if chk==0 then return ft>0
		and Duel.IsExistingMatchingCard(c21223277.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp,e:GetHandler()) end
...
function c21223277.nbarmsfilters(c)
	return c:IsFaceup() and c:IsEquipSpell() and c:IsSetCard(SET_NOBLE_ARMS)
end
function c21223277.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c21223277.nbarmsfilters,tp,LOCATION_SZONE,0,1,nil)
...
function c21223277.desop(e,tp,eg,ep,ev,re,r,rp)
	local ct=Duel.GetMatchingGroupCount(c21223277.nbarmsfilters,tp,LOCATION_SZONE,0,nil)
