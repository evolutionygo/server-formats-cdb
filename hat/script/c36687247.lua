...
function c36687247.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if (c:GetPreviousLocation()&LOCATION_ONFIELD)~=0 and c:IsReason(REASON_DESTROY) then
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc36687247(c36687247,0))
...
function c36687247.filter(c)
	return c:IsSetCard(0x27) and c:GetCode()~=c36687247 and c:IsMonster() and c:IsAbleToHand()
end
