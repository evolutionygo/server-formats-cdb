...
function c53670497.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc53670497(c53670497,1))
	...
	e3:SetDescription(aux.Stringc53670497(c53670497,2))
...
function c53670497.target1(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if ((turnp~=tp and Duel.IsExistingMatchingCard(c53670497.filter1,tp,LOCATION_DECK,0,1,nil))
		...
		and Duel.SelectYesNo(tp,aux.Stringc53670497(c53670497,0)) then
		if turnp==tp then
		...
		e:SetLabel(1)
		e:GetHandler():RegisterFlagEffect(c53670497,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	end
end
function c53670497.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	local turnp=Duel.GetTurnPlayer()
	if chk==0 then return e:GetHandler():GetFlagEffect(c53670497)==0
