...
c67949763.listed_names={67949764}
function c67949763.filter(c,tp)
	return c:IsFaceup() and c:IsType(TYPE_XYZ)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,c67949763+1,0x87,TYPES_TOKEN,-2,0,1,RACE_FIEND,ATTRIBUTE_DARK)
end
...
function c67949763.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>2 then
		local rfc67949763=tc:GetRealFieldID()
		local atk=0
		local cr=false
		...
		if not Duel.IsPlayerCanSpecialSummonMonster(tp,c67949763+1,0x87,TYPES_TOKEN,-2,0,1,RACE_FIEND,ATTRIBUTE_DARK) then return end
		if cr then
			...
			de:SetLabel(rfc67949763)
			tc:RegisterEffect(de,true)
		end
		for i=1,3 do
			local token=Duel.CreateToken(tp,c67949763+1)
			...
			if cr then
				token:RegisterFlagEffect(c67949763+1,RESET_EVENT+RESETS_STANDARD,0,0,rfc67949763)
...
function c67949763.desfilter(c,rfc67949763)
	return c:GetFlagEffectLabel(c67949763+1)==rfc67949763
end
