...
function c4756629.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc4756629(c4756629,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCountLimit(1,c4756629)
	...
	e2:SetDescription(aux.Stringc4756629(c4756629,1))
...
function c4756629.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() then
		c:RegisterFlagEffect(c4756629,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	end
end
function c4756629.sumcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetFlagEffect(c4756629)~=0
end
...
function c4756629.decop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.GetFlagEffect(tp,c4756629+1)~=0 then return end
	local e1=Effect.CreateEffect(c)
	...
	c:RegisterEffect(e2)
	Duel.RegisterFlagEffect(tp,c4756629+1,RESET_PHASE+PHASE_END,0,1)
end
function c4756629.econ(e)
	return #{Duel.GetPlayerEffect(e:GetHandlerPlayer(),c4756629)}~=0
end
