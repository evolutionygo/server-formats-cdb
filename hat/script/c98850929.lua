...
function c98850929.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc98850929(c98850929,4))
	e1:SetCategory(CATEGORY_TOHAND)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCountLimit(1,c98850929,EFFECT_COUNT_CODE_OATH)
...
function c98850929.filter3(c,e)
	return c:IsSpell() and c:GetCode()~=c98850929 and c:IsAbleToHand() and c:IsCanBeEffectTarget(e)
end
function c98850929.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	repeat
		...
		if ct>1 and #g1>0 then
			ops[off]=aux.Stringc98850929(c98850929,0)
			opval[off-1]=1
			off=off+1
		end
		if ct>2 and #g2>0 then
			ops[off]=aux.Stringc98850929(c98850929,1)
			opval[off-1]=2
			off=off+1
		end
		if ct>3 and #g3>0 then
			ops[off]=aux.Stringc98850929(c98850929,2)
			opval[off-1]=3
			off=off+1
		end
	...
	until off<3 or not Duel.SelectYesNo(tp,aux.Stringc98850929(c98850929,3))
