...
function c581014.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc581014(c581014,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1)
	e1:SetCost(aux.dxmcostgen(1,1,nil))
	e1:SetTarget(c581014.efftg)
	e1:SetOperation(c581014.effop)
	c:RegisterEffect(e1,false,REGISTER_FLAG_DETACH_XMAT)
end
function c581014.spfilter(c,e,tp)
	return c:IsNonEffectMonster() and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c581014.efftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		...
		elseif op==2 then
			return c581014.spfilter(chkc,e,tp)
		end
	end
	local b1=Duel.IsPlayerCanDraw(tp,1)
		and Duel.IsExistingTarget(aux.AND(Card.IsMonster,Card.IsAbleToDeck),tp,LOCATION_GRAVE,0,3,nil)
	local b2=Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c581014.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp)
	if chk==0 then return b1 or b2 end
	local op=Duel.SelectEffect(tp,
		{b1,aux.Stringc581014(c581014,1)},
		{b2,aux.Stringc581014(c581014,2)})
	e:SetLabel(op)
	if op==1 then
	...
	elseif op==2 then
		...
		local g=Duel.SelectTarget(tp,c581014.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
		Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,1,tp,0)
	end
end
function c581014.effop(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
