...
function c11221418.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)>0 then
		tc:RegisterFlagEffect(c11221418,RESET_EVENT+RESETS_STANDARD,0,0)
		local g=Duel.GetMatchingGroup(Card.IsAbleToHand,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
		if #g>0 then Duel.SendtoHand(g,nil,REASON_EFFECT) end
		--trigger effect
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc11221418(c11221418,0))
		e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
		e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
		e1:SetCode(EVENT_PHASE+PHASE_END)
		e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
		e1:SetCountLimit(1)
		e1:SetCondition(c11221418.spresetcon)
		e1:SetTarget(c11221418.sptg)
		e1:SetOperation(c11221418.spop)
		e1:SetLabel(rct)
	...
	e7:SetDescription(aux.Stringc11221418(c11221418,2))
	...
	e8:SetDescription(aux.Stringc11221418(c11221418,1))
	e8:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e8:SetCode(EVENT_PHASE+PHASE_END)
	e8:SetCountLimit(1)
	e8:SetCondition(c11221418.spresetcon)
	e8:SetOperation(c11221418.resetall)
	e8:SetLabel(rct)
...
function c11221418.spresetcon(e,tp,eg,ep,ev,re,r,rp)
...
function c11221418.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:GetFlagEffect(c11221418)>0 and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)>0
