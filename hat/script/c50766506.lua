...
c50766506.listed_series={0x2b}
function c50766506.cfilter(c,e,tp)
	local lv=c:GetLevel()
	return lv>0 and c:IsSetCard(0x2b) and Duel.GetMZoneCount(tp,c)>0 and Duel.IsExistingMatchingCard(c50766506.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,lv)
end
function c50766506.spfilter(c,e,tp,lv)
...
function c50766506.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroupCost(tp,c50766506.cfilter,1,false,nil,nil,e,tp) end
	local rg=Duel.SelectReleaseGroupCost(tp,c50766506.cfilter,1,1,false,nil,nil,e,tp)
	e:SetLabel(rg:GetFirst():GetLevel())
	Duel.Release(rg,REASON_COST)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c50766506.rescon(sg,e,tp,mg)
...
function c50766506.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local lv=e:GetLabel()
	local g=Duel.GetMatchingGroup(c50766506.spfilter,tp,LOCATION_DECK,0,nil,e,tp,lv)
	if #g==0 then return end
	local sg=aux.SelectUnselectGroup(g,e,tp,1,ft,c50766506.rescon,1,tp,HINTMSG_SPSUMMON)
