...
function c923596.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc923596(c923596,0))
	...
	e2:SetDescription(aux.Stringc923596(c923596,1))
...
c923596.listed_series={0x72}
c923596.listed_names={c923596}
function c923596.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsCanTurnSet() and c:GetFlagEffect(c923596)==0 end
	c:RegisterFlagEffect(c923596,RESET_EVENT+RESETS_STANDARD-RESET_TURN_SET+RESET_PHASE+PHASE_END,0,1)
...
function c923596.filter(c)
	return c:IsSetCard(0x72) and not c:IsCode(c923596) and c:IsMonster() and c:IsAbleToHand()
end
