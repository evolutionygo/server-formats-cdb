...
function c94256039.initial_effect(c)
	...
	e4:SetDescription(aux.Stringc94256039(c94256039,0))
	e4:SetCategory(CATEGORY_DESTROY+CATEGORY_DAMAGE)
	e4:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e4:SetCode(EVENT_CUSTOM+c94256039)
...
c94256039.counter_place_list={COUNTER_SPELL}
function c94256039.acop(e,tp,eg,ep,ev,re,r,rp)
	...
	if re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_SPELL) and c:GetFlagEffect(1)>0 then
		...
		if c:GetCounter(COUNTER_SPELL)==4 then
			Duel.RaiseSingleEvent(c,EVENT_CUSTOM+c94256039,re,0,0,p,0)
		end
	end
end
