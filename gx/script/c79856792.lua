...
function c79856792.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc79856792(c79856792,0))
	...
	e3:SetCondition(c79856792.atkcon)
	e3:SetCost(c79856792.atkcost)
	e3:SetOperation(c79856792.atkop)
	c:RegisterEffect(e3)
	--Shuffle all cards on the field into the Deck
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc79856792(c79856792,1))
	e4:SetCategory(CATEGORY_TODECK)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCondition(function(e) return not e:GetHandler():HasFlagEffect(c79856792) end)
...
c79856792.listed_series={SET_CRYSTAL_BEAST}
...
function c79856792.spop(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():RegisterFlagEffect(c79856792,RESET_EVENT|RESETS_STANDARD&~(RESET_TOFIELD|RESET_TURN_SET)|RESET_PHASE|PHASE_END,0,1)
end
function c79856792.atkcon(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():HasFlagEffect(c79856792) then return false end
	return Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamageCalculated()
end
function c79856792.atkfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_CRYSTAL_BEAST) and c:IsAbleToGraveAsCost()
end
function c79856792.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(c79856792.atkfilter,tp,LOCATION_MZONE,0,nil)
	if chk==0 then return #g>0 and not g:IsContains(e:GetHandler()) end
	Duel.SendtoGrave(g,REASON_COST)
	e:SetLabel(#g)
end
function c79856792.atkop(e,tp,eg,ep,ev,re,r,rp)
