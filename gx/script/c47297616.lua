...
function c47297616.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc47297616(c47297616,0))
	...
	e2:SetTarget(c47297616.negtg)
	e2:SetOperation(c47297616.negop)
	c:RegisterEffect(e2)
	--Destroy all cards you control and Special Summon 1 monster from your GY
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc47297616(c47297616,1))
	e3:SetCategory(CATEGORY_DESTROY+CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e3:SetCode(EVENT_TO_GRAVE)
	e3:SetCondition(function(e) return e:GetHandler():IsReason(REASON_DESTROY) end)
	e3:SetTarget(c47297616.sptg)
	e3:SetOperation(c47297616.spop)
	c:RegisterEffect(e3)
	--"Double Snare" valc47297616ity
	aux.DoubleSnareValc47297616ity(c,LOCATION_MZONE)
end
function c47297616.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return not c:HasFlagEffect(c47297616) end
	if c:IsHasEffect(EFFECT_REVERSE_UPDATE) then
		c:RegisterFlagEffect(c47297616,RESETS_STANDARD_PHASE_END,0,1)
	end
	Duel.SetOperationInfo(0,CATEGORY_NEGATE,eg,1,tp,0)
end
function c47297616.negop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c47297616.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c47297616.spop(e,tp,eg,ep,ev,re,r,rp)
