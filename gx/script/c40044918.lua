...
function c40044918.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc40044918(c40044918,0))
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_SUMMON_SUCCESS)
	e1:SetTarget(c40044918.efftg)
	e1:SetOperation(c40044918.effop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	c:RegisterEffect(e2)
end
c40044918.listed_series={SET_HERO}
function c40044918.thfilter(c)
	return c:IsSetCard(SET_HERO) and c:IsMonster() and c:IsAbleToHand()
end
function c40044918.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b2=Duel.IsExistingMatchingCard(c40044918.thfilter,tp,LOCATION_DECK,0,1,nil)
	if chk==0 then return b1 or b2 end
	local op=Duel.SelectEffect(tp,
		{b1,aux.Stringc40044918(c40044918,1)},
		{b2,aux.Stringc40044918(c40044918,2)})
	e:SetLabel(op)
	if op==1 then
...
function c40044918.effop(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
	...
	elseif op==2 then
		...
		local g=Duel.SelectMatchingCard(tp,c40044918.thfilter,tp,LOCATION_DECK,0,1,1,nil)
		if #g>0 then
