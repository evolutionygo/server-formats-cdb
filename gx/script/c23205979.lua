...
function c23205979.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc23205979(c23205979,0))
	...
	e6:SetOperation(c23205979.register)
	c:RegisterEffect(e6)
end
...
function c23205979.register(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():RegisterFlagEffect(c23205979+1,RESET_CHAIN,0,1,ev)
end
function c23205979.desop1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetChainInfo(ev,CHAININFO_TARGET_CARDS)
	if not g or not g:IsContains(c) then return false end
	for _,ch in ipairs({c:GetFlagEffectLabel(c23205979+1)}) do
		if ch==ev then
			if (Duel.GetCurrentPhase()&(PHASE_DAMAGE|PHASE_DAMAGE_CAL))~=0 and not Duel.IsDamageCalculated() then
				c:RegisterFlagEffect(c23205979,RESET_PHASE|PHASE_DAMAGE|RESETS_STANDARD,0,1)
...
function c23205979.desop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:GetFlagEffect(c23205979)>0 and not c:IsHasEffect(EFFECT_DISABLE) and not c:IsDisabled() then
		Duel.Destroy(c,REASON_EFFECT)
	end
end