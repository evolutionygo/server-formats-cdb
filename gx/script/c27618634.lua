...
function c27618634.atop(e,tp,eg,ep,ev,re,r,rp)
	...
	if bc and c:IsPosition(POS_FACEUP_ATTACK) then
		if c:GetFlagEffect(c27618634)==0 then
			c:RegisterFlagEffect(c27618634,RESET_EVENT+RESETS_STANDARD,0,1)
			e:GetLabelObject():Clear()
		end
		e:GetLabelObject():AddCard(bc)
		bc:RegisterFlagEffect(c27618634+1,RESET_EVENT+RESETS_STANDARD,0,1)
	end
end
function c27618634.atlcon(e)
	return e:GetHandler():GetFlagEffect(c27618634)~=0
end
function c27618634.atltg(e,c)
	return e:GetLabelObject():IsContains(c) and c:GetFlagEffect(c27618634+1)~=0
end
function c27618634.posop(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():ResetFlagEffect(c27618634)
end