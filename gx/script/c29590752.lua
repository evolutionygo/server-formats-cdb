...
function c29590752.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc29590752(c29590752,0))
	...
	e2:SetDescription(aux.Stringc29590752(c29590752,1))
...
function c29590752.dtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetFlagEffect(c29590752+1)==0 end
	e:GetHandler():RegisterFlagEffect(c29590752+1,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_BATTLE,EFFECT_FLAG_OATH,1)
	Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,1,tp,0)
end
function c29590752.dop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsFaceup() or not c:IsRelateToEffect(e) then return end
	if Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)>0 and Duel.SelectYesNo(tp,aux.Stringc29590752(c29590752,2)) then
