...
function c34487429.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc34487429(c34487429,0))
	e3:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_FIELD)
	e3:SetRange(LOCATION_FZONE)
	e3:SetCode(EVENT_PRE_BATTLE_DAMAGE)
	e3:SetCondition(c34487429.rdcon)
	e3:SetOperation(c34487429.rdop)
	c:RegisterEffect(e3)
	--Negate Spell/Trap activation
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc34487429(c34487429,1))
	e4:SetCategory(CATEGORY_NEGATE+CATEGORY_DESTROY)
	e4:SetType(EFFECT_TYPE_QUICK_O)
	e4:SetCode(EVENT_CHAINING)
	e4:SetRange(LOCATION_FZONE)
	e4:SetCondition(c34487429.discon)
	e4:SetCost(c34487429.discost)
	e4:SetTarget(c34487429.distg)
	e4:SetOperation(c34487429.disop)
	c:RegisterEffect(e4)
	--Draw 1 card
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringc34487429(c34487429,2))
	e5:SetCategory(CATEGORY_DRAW)
	e5:SetType(EFFECT_TYPE_IGNITION)
	e5:SetRange(LOCATION_FZONE)
	e5:SetCountLimit(1)
	e5:SetCondition(c34487429.drcon)
	e5:SetTarget(c34487429.drtg)
	e5:SetOperation(c34487429.drop)
	c:RegisterEffect(e5)
	--Special Summon 1 "Crystal Beast" monster from the Spell/Trap Zone
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringc34487429(c34487429,3))
	e6:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e6:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e6:SetType(EFFECT_TYPE_IGNITION)
	e6:SetRange(LOCATION_FZONE)
	e6:SetCountLimit(1)
	e6:SetCondition(c34487429.spcon)
	e6:SetTarget(c34487429.sptg)
	e6:SetOperation(c34487429.spop)
	c:RegisterEffect(e6)
	aux.DoubleSnareValc34487429ity(c,LOCATION_SZONE)
end
c34487429.listed_series={0x1034}
...
function c34487429.rdcon(e,tp,eg,ep,ev,re,r,rp)
	return ep==tp and e:GetHandler():GetFlagEffect(c34487429)==0
...
function c34487429.rdop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.SelectEffectYesNo(tp,e:GetHandler()) then
		Duel.Hint(HINT_CARD,1-tp,c34487429)
		Duel.ChangeBattleDamage(tp,math.floor(ev/2))
		e:GetHandler():RegisterFlagEffect(c34487429,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	end
end
