...
function c1248895.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc1248895(c1248895,0))
...
function c1248895.tgfilter(c,e,tp)
	...
	if c:IsControler(tp) then
		...
		return Duel.GetFieldGroupCount(c:GetControler(),LOCATION_HAND|LOCATION_DECK,0)>0 and c1248895.mdnamecheck(c)
	end
end
function c1248895.mdnamecheck(c)
...
function c1248895.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local dg=Duel.GetMatchingGroup(c1248895.publicfilter,ctrl_player,LOCATION_HAND|LOCATION_DECK,0,nil,convulsion,top_card,tc:GetCode())
	if #dg>0 then
