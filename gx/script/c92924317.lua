...
function c92924317.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc92924317(c92924317,0))
	...
	e3:SetOperation(c92924317.mondesop)
	...
	e4:SetCondition(c92924317.selfdescon)
	e4:SetOperation(function(e) Duel.Destroy(e:GetHandler(),REASON_EFFECT) end)
	c:RegisterEffect(e4)
end
function c92924317.spfilter(c,e,tp)
...
function c92924317.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c92924317.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c92924317.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c92924317.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c92924317.mondesop(e,tp,eg,ep,ev,re,r,rp)
...
function c92924317.selfdescon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetHandler():GetFirstCardTarget()
