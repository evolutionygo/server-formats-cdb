...
function c13857930.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc13857930(c13857930,0))
...
function c13857930.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetFlagEffect(c13857930)==0 end
	e:GetHandler():RegisterFlagEffect(c13857930,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_DAMAGE,0,1)
...
function c13857930.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif tc:IsSpell() then
		if c==Duel.GetAttacker() and not c:IsHasEffect(EFFECT_CANNOT_DIRECT_ATTACK)
			and c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SelectYesNo(tp,aux.Stringc13857930(c13857930,1)) then
