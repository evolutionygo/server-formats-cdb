...
function c58332301.initial_effect(c)
	c:EnableReviveLimit()
	--Fusion material
	aux.AddFusionProcCode2(c,true,true,aux.FilterBoolFunctionEx(Card.IsRace,RACE_FIEND),aux.FilterBoolFunctionEx(Card.IsRace,RACE_ROCK))
	--lizard check
	local e0=Effect.CreateEffect(c)
	e0:SetType(EFFECT_TYPE_SINGLE)
	e0:SetCode(CARD_CLOCK_LIZARD)
	e0:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e0:SetCondition(c58332301.lizcon)
	e0:SetValue(1)
	c:RegisterEffect(e0)
	...
	e2:SetOperation(c58332301.atkop)
	c:RegisterEffect(e2)
	--Change battle position
	local e3=Effect.CreateEffect(c)
	e3:SetCategory(CATEGORY_POSITION)
	e3:SetDescription(aux.Stringc58332301(c58332301,0))
...
c58332301.listed_names={CARD_DARK_FUSION}
function c58332301.lizcon(e,tp,eg,ep,ev,re,r,rp)
