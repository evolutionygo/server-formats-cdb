...
function c39980304.activate(e,tp,eg,ep,ev,re,r,rp)
	local e1=Effect.CreateEffect(e:GetHandler())
	e1:SetDescription(aux.Stringc39980304(c39980304,0))
	e1:SetType(EFFECT_TYPE_FIELD)
...
function c39980304.chain_target(e,te,tp,value)
...
function c39980304.chain_operation(e,te,tp,tc,mat,sumtype,sg,sumpos)
	...
	if sg then
		sg:AddCard(tc)
		tc:RegisterFlagEffect(c39980304,RESET_EVENT+RESETS_STANDARD-(RESET_TOFIELD+RESET_TURN_SET),0,1)
	else
		Duel.SpecialSummonStep(tc,sumtype,tp,tp,false,false,sumpos)
		tc:RegisterFlagEffect(c39980304,RESET_EVENT+RESETS_STANDARD-RESET_TURN_SET,0,1)
	end
	local e1=Effect.CreateEffect(e:GetHandler())
...
function c39980304.descon(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:GetFlagEffect(c39980304)~=0 then
		return true
	else
		e:Reset()
		return false
