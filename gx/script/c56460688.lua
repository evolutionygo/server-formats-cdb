...
function c56460688.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Remove(tg,0,REASON_EFFECT+REASON_TEMPORARY)~=0 then
		...
		for tc in aux.Next(g) do
			tc:RegisterFlagEffect(c56460688,RESET_EVENT+RESETS_STANDARD,0,1)
		end
		c:RegisterFlagEffect(c56460688,RESET_EVENT+RESETS_STANDARD_EXC_GRAVE,0,1)
		g:KeepAlive()
		e:SetLabelObject(g)
	end
end
function c56460688.retop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():GetFlagEffect(c56460688)==0 then return end
	if (r&REASON_DESTROY)~=0 then
		local g=e:GetLabelObject():GetLabelObject()
		local tc=g:GetFirst()
		for tc in aux.Next(g) do
			if tc:GetFlagEffect(c56460688)>0 then
