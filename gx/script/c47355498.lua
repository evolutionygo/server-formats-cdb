...
function c47355498.initial_effect(c)
	...
	e11:SetTarget(c47355498.cannotsptg)
	c:RegisterEffect(e11)
end
c47355498.listed_series={SET_GRAVEKEEPERS}
...
function c47355498.disfilter(c,not_im0,not_im1,re)
...
function c47355498.discheck(ev,category,re,not_im0,not_im1)
	...
	if tg and #tg>0 then
		return tg:IsExists(c47355498.disfilter,1,nil,not_im0,not_im1,re)
	end
	return false
end
function c47355498.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not res and c47355498.discheck(ev,CATEGORY_SPECIAL_SUMMON,re,not_im0,not_im1) then res=true end
	if not res and c47355498.discheck(ev,CATEGORY_REMOVE,re,not_im0,not_im1) then res=true end
	if not res and c47355498.discheck(ev,CATEGORY_TOHAND,re,not_im0,not_im1) then res=true end
	if not res and c47355498.discheck(ev,CATEGORY_TODECK,re,not_im0,not_im1) then res=true end
	if not res and c47355498.discheck(ev,CATEGORY_TOEXTRA,re,not_im0,not_im1) then res=true end
	if not res and c47355498.discheck(ev,CATEGORY_EQUIP,re,not_im0,not_im1) then res=true end
	if not res and c47355498.discheck(ev,CATEGORY_LEAVE_GRAVE,re,not_im0,not_im1) then res=true end
	if res then Duel.NegateEffect(ev) end
end
function c47355498.cannotsptg(e,c,sp,sumtype,sumpos,target_p,sumeff)
