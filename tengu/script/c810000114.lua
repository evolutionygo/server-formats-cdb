...
function c810000114.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc810000114(c810000114,0))
...
function c810000114.checkop(e,tp,eg,ep,ev,re,r,rp)
	if eg:IsExists(c810000114.cfilter,1,nil,0) then
		Duel.RegisterFlagEffect(0,c810000114,RESET_PHASE+PHASE_END,0,2)
	end
	if eg:IsExists(c810000114.cfilter,1,nil,1) then
		Duel.RegisterFlagEffect(1,c810000114,RESET_PHASE+PHASE_END,0,2)
	end
end
function c810000114.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,c810000114)>0 end
