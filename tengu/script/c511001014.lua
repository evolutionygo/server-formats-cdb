...
c511001014.listed_series={0xba}
function c511001014.filter(c,e,tp,tc511001014)
	local rk=c:GetRank()
	local pg=aux.GetMustBeMaterialGroup(tp,Group.FromCards(c),tp,nil,nil,REASON_XYZ)
	return #pg<=1 and c:GetTurnID()==tc511001014 and c:GetReason()&REASON_BATTLE~=0
...
function c511001014.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local tc511001014=Duel.GetTurnCount()
	if chkc then return chkc:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED) and chkc:IsControler(tp) and c511001014.filter(chkc,e,tp,tc511001014) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonCount(tp,2) 
		and Duel.IsExistingTarget(c511001014.filter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp,tc511001014) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c511001014.filter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,e,tp,tc511001014)
