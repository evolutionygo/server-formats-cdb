...
function c513000125.initial_effect(c)
	...
	e4:SetDescription(aux.Stringc513000125(c:Alias(),1))
...
function c513000125.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(e:GetLabelObject()) do
		...
		c:RegisterFlagEffect(c513000125,RESET_EVENT+RESETS_STANDARD_DISABLE,0,1)
	end
end
function c513000125.chkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local dz=c:GetCounter(0x1110)>0 and c:GetFlagEffect(c513000125)~=c:GetCounter(0x1110)
	if (tg or dz) and e:GetLabel()==0 then Duel.Hint(HINT_CARD,tp,c513000125) end
	if tg then
	...
	if dz then
		...
		local ft=c:GetCounter(0x1110)-c:GetFlagEffect(c513000125)
		if ft<0 then
