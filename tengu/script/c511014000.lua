...
function c511014000.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511014000(c511014000,0))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_TOKEN)
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e2:SetRange(LOCATION_FZONE)
	e2:SetCode(EVENT_CUSTOM+c511014000)
	e2:SetTarget(c511014000.tktg)
	e2:SetOperation(c511014000.tkop)
	c:RegisterEffect(e2)
	--Special Summon 1 monster from the GY
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc511014000(c511014000,1))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_FZONE)
	e3:SetCost(c511014000.spcost)
	e3:SetTarget(c511014000.sptg)
	e3:SetOperation(c511014000.spop)
	c:RegisterEffect(e3)
...
function c511014000.regcon(e,tp,eg,ep,ev,re,r,rp)
	...
	e:SetLabel(sf)
	return sf~=0 --and (not re or not re:GetHandler():IsCode(c511014000,71645242))
end
function c511014000.regop(e,tp,eg,ep,ev,re,r,rp)
	Duel.RaiseEvent(eg,EVENT_CUSTOM+c511014000,e,r,rp,ep,e:GetLabel())
end
