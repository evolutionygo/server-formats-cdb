...
function c1315120.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if (c:GetPreviousLocation()&LOCATION_ONFIELD)~=0 and c:IsReason(REASON_DESTROY) then
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc1315120(c1315120,0))
...
function c1315120.filter(c)
	return c:IsSetCard(0x27) and c:GetCode()~=c1315120 and c:IsMonster() and c:IsAbleToHand()
end
