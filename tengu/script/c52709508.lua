...
function c52709508.initial_effect(c)
	--Synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_GENEX),1,1,aux.NonTuner(nil),1,99)
	c:EnableReviveLimit()
...
c52709508.listed_series={SET_GENEX}
...
function c52709508.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if (att&ATTRIBUTE_EARTH)~=0 then
		...
		c:RegisterEffect(e1)
		c:RegisterFlagEffect(0,RESET_EVENT|RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringc52709508(c52709508,2))
	end
	if (att&ATTRIBUTE_FIRE)~=0 then
		--Inflict damage when it destroys a monster by battle
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc52709508(c52709508,0))
		...
		e1:SetReset(RESET_EVENT|RESETS_STANDARD)
		c:RegisterEffect(e1)
		c:RegisterFlagEffect(0,RESET_EVENT|RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringc52709508(c52709508,3))
	end
	if (att&ATTRIBUTE_LIGHT)~=0 then
		--Special Summon a LIGHT monster from the GY
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc52709508(c52709508,1))
		...
		e1:SetReset(RESET_EVENT|RESETS_STANDARD)
		c:RegisterEffect(e1)
		c:RegisterFlagEffect(0,RESET_EVENT|RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringc52709508(c52709508,4))
	end
end
