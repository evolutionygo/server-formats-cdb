...
function c94303232.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc94303232(c94303232,0))
...
function c94303232.checkop(e,tp,eg,ep,ev,re,r,rp)
	for tc in eg:Iter() do
		if tc:IsPreviousPosition(POS_FACEUP_ATTACK) and tc:IsPosition(POS_FACEUP_DEFENSE) then
			tc:RegisterFlagEffect(c94303232,RESETS_STANDARD_PHASE_END,0,1)
		end
	end
end
function c94303232.eqlimit(e,c)
	return c:HasFlagEffect(c94303232) or c:HasFlagEffect(c94303232+1)
end
function c94303232.filter(c)
	return c:IsFaceup() and c:HasFlagEffect(c94303232)
end
function c94303232.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,c94303232.filter,tp,LOCATION_MZONE,0,1,1,nil)
...
function c94303232.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and c:IsRelateToEffect(re) and tc:IsRelateToEffect(re) and tc:IsFaceup() and Duel.Equip(tp,c,tc) then
		tc:RegisterFlagEffect(c94303232+1,RESET_EVENT|RESETS_STANDARD,0,1)
	end
end
