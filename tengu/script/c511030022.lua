...
--Draw Discharge
--scripted by pyrQ
function c511030022.initial_effect(c)
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DAMAGE+CATEGORY_REMOVE+CATEGORY_HANDES)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e1:SetCode(EVENT_DRAW)
	e1:SetCondition(c511030022.condition)
	e1:SetTarget(c511030022.target)
	e1:SetOperation(c511030022.activate)
	c:RegisterEffect(e1)
end
function c511030022.condition(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and Duel.GetCurrentPhase()~=PHASE_DRAW
end
function c511030022.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511030022.activate(e,tp,eg,ep,ev,re,r,rp)
