...
function c511002896.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511002896(c511002896,0))
	...
	e1:SetTarget(c511002896.atktg)
	e1:SetOperation(c511002896.atkop)
	c:RegisterEffect(e1,false,REGISTER_FLAG_DETACH_XMAT)
	--Draw 1 card
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc511002896(c511002896,1))
...
function c511002896.atkfilter(c)
	return c:IsType(TYPE_XYZ) and not c:IsAttack(0) and c:IsFaceup()
end
function c511002896.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_MZONE) and c511002896.atkfilter(chkc) end
	local g=Duel.GetMatchingGroup(Card.IsNegatable,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,e:GetHandler())
	if chk==0 then return #g>0 and Duel.IsExistingTarget(c511002896.atkfilter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATKDEF)
	local tc=Duel.SelectTarget(tp,c511002896.atkfilter,tp,0,LOCATION_MZONE,1,1,nil):GetFirst()
	Duel.SetOperationInfo(0,CATEGORY_ATKCHANGE,tc,1,tp,-tc:GetAttack())
	Duel.SetOperationInfo(0,CATEGORY_DISABLE,g,1,tp,0)
end
function c511002896.atkop(e,tp,eg,ep,ev,re,r,rp)
