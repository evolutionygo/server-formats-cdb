...
function c511001429.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511001429(c511001429,0))
	e2:SetCategory(CATEGORY_ATKCHANGE+CATEGORY_DISABLE)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCost(aux.dxmcostgen(1,1,nil))
	e2:SetTarget(c511001429.tg)
	e2:SetOperation(c511001429.op)
	--indes
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc511001429(c511001429,1))
	e3:SetCategory(CATEGORY_DAMAGE)
	e3:SetType(EFFECT_TYPE_QUICK_O)
	e3:SetCode(EVENT_PRE_DAMAGE_CALCULATE)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCountLimit(1,0,EFFECT_COUNT_CODE_SINGLE)
	e3:SetCondition(c511001429.indescon)
	e3:SetCost(c511001429.indescost)
	e3:SetTarget(c511001429.indestg)
	e3:SetOperation(c511001429.indesop)
	local e4=e3:Clone()
	e4:SetCode(EVENT_CHAINING)
	e4:SetCondition(c511001429.indescon2)
...
c511001429.listed_series={SET_NUMBER}
c511001429.aux.xyz_number=102
...
function c511001429.op(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,c511001429.filter,tp,0,LOCATION_MZONE,1,1,nil):GetFirst()
	if tc then
