...
c513000166.listed_names={68140974,100000051,100000052,100000053,100000054}
...
function c513000166.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ft<=0 or (ft>1 and Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)) then return end
 	local sg=Duel.GetMatchingGroup(c513000166.filter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE,0,nil,e,tp)
 	if not c513000166.rescon(sg) then return end
 	local spg=aux.SelectUnselectGroup(sg,e,tp,5,5,c513000166.rescon,1,tp,HINTMSG_SPSUMMON)
	if #sg>0 then
