...
function c511600090.initial_effect(c)
	...
	e5:SetDescription(aux.Stringc511600090(16306932,2))
...
function c511600090.rescon(sg,e,tp,mg)
	return aux.ChkfMMZ(1)(sg,e,tp,mg) and sg:IsExists(c511600090.chk,1,nil,sg,Group.CreateGroup(),41209827,82044279,16195942)
end
function c511600090.chk(c,sg,g,code,...)
	if not c:IsCode(code) then return false end
	local res
	if ... then
		g:AddCard(c)
		res=sg:IsExists(c511600090.chk,1,g,sg,g,...)
		g:RemoveCard(c)
	else
		res=true
	end
...
function c511600090.hspcon(e,c)
	...
	g:Merge(g2)
	g:Merge(g3)
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3 and #g1>0 and #g2>0 and #g3>0 and aux.SelectUnselectGroup(g,e,tp,3,3,c511600090.rescon,0)
end
function c511600090.hsptg(e,tp,eg,ep,ev,re,r,rp,c)
	local rg=Duel.GetReleaseGroup(tp)
	local g1=rg:Filter(Card.IsCode,nil,41209827)
	local g2=rg:Filter(Card.IsCode,nil,82044279)
	local g3=rg:Filter(Card.IsCode,nil,16195942)
	g1:Merge(g2)
	g1:Merge(g3)
	local g1=aux.SelectUnselectGroup(g1,e,tp,3,3,c511600090.rescon,1,tp,HINTMSG_RELEASE,c511600090.rescon,nil,true)
	if #g1>0 then
		g1:KeepAlive()
		e:SetLabelObject(g1)
		return true
