...
function c1347977.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc1347977(c1347977,0))
...
function c1347977.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	e:SetLabel(0)
	local dhg=Duel.GetMatchingGroup(c1347977.tgfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,e)
	local b1=dhg:FilterCount(Card.IsAbleToDeck,nil)>0
	local b2=Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsRace,RACE_WARRIOR),tp,LOCATION_MZONE,0,1,nil) and aux.SelectUnselectGroup(dhg,e,tp,2,2,c1347977.rescon,0)
	if not (b1 or b2) then return end
	local op=Duel.SelectEffect(tp,
		{b1,aux.Stringc1347977(c1347977,1)},
		{b2,aux.Stringc1347977(c1347977,2)})
	e:SetLabel(op)
	...
	elseif op==2 then
		tg=aux.SelectUnselectGroup(dhg,e,tp,2,2,c1347977.rescon,1,tp,HINTMSG_TARGET)
...
function c1347977.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif op==2 then
		...
		if #tg>1 then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
			dc=tg:FilterSelect(tp,c1347977.selfilter,1,1,nil,tg):GetFirst()
...
function c1347977.selfilter(c,g)
