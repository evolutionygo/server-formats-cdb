...
function c511000118.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511000118(c511000118,0))
	...
	e2:SetDescription(aux.Stringc511000118(c511000118,1))
...
function c511000118.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return not e:GetHandler():HasFlagEffect(c511000118) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,1,c)
	end
	c:RegisterFlagEffect(c511000118,RESET_EVENT|(RESETS_STANDARD&~(RESET_TOFIELD|RESET_LEAVE))|RESET_PHASE|PHASE_END,0,1)
...
function c511000118.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	c:RegisterEffect(e2)
	c:RegisterFlagEffect(c511000118+1,RESET_EVENT|RESETS_STANDARD,0,1)
end
function c511000118.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and not c:HasFlagEffect(c511000118) and c:HasFlagEffect(c511000118+1) end
	c:RegisterFlagEffect(c511000118,RESET_EVENT|(RESETS_STANDARD&~(RESET_TOFIELD|RESET_LEAVE))|RESET_PHASE|PHASE_END,0,1)
