...
function c513000141.initial_effect(c)
	...
	e4:SetDescription(aux.Stringc513000141(c513000141,0))
...
c513000141.aux.xyz_number=69
function c513000141.copfilter(c)
	return c:IsFaceup() and c:IsStatus(STATUS_DISABLED) and c:GetFlagEffect(c513000141)==0
end
function c513000141.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	for wbc in aux.Next(wg) do
		if c:IsFaceup() then
			local cc513000141=c:CopyEffect(wbc:GetOriginalCode(),RESET_EVENT+RESETS_STANDARD_DISABLE,1)
			wbc:RegisterFlagEffect(c513000141,0,0,0,cc513000141)
		end
	end
end
function c513000141.rfilter(c)
	return c:GetFlagEffect(c513000141)>0
end
function c513000141.reset(e,tp,eg,ep,ev,re,r,rp)
	local wg=eg:Filter(c513000141.rfilter,nil)
	for wbc in aux.Next(wg) do
		e:GetHandler():ResetEffect(wbc:GetFlagEffectLabel(c513000141),RESET_COPY)
		wbc:ResetFlagEffect(c513000141)
	end
end
