...
function c24104865.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc24104865(c24104865,0))
...
c24104865.counter_list={COUNTER_A}
...
function c24104865.checkop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if aux.bdgcon(e,tp,eg,ep,ev,re,r,rp) then
		g:AddCard(t)
		t:RegisterFlagEffect(c24104865,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_BATTLE,0,1)
	end
end
...
function c24104865.filter(c,e,tp)
	return c:GetFlagEffect(c24104865)~=0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function c24104865.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(sg) do
		...
		tc:RegisterFlagEffect(c24104865,RESET_EVENT+RESETS_STANDARD,0,0)
		c:CreateRelation(tc,RESET_EVENT+0x1020000)
	end
	Duel.SpecialSummonComplete()
end
function c24104865.desfilter(c,rc)
	return c:GetFlagEffect(c24104865)~=0 and rc:IsRelateToCard(c)
end
