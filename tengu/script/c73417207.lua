...
function c73417207.chk2(c)
	...
	for i=1,#te do
		local e=te[i]
		if e:GetLabel()==c73417207 then return true end
	end
	return false
end
function c73417207.filterchk(c)
	...
	return not c:IsHasEffect(EFFECT_HAND_SYNCHRO) or c:IsHasEffect(EFFECT_HAND_SYNCHRO+EFFECT_SYNCHRO_CHECK) 
		or c:GetCardEffect(EFFECT_HAND_SYNCHRO):GetLabel()~=c73417207
end
function c73417207.synchktg(e,c,sg,tg,ntg,tsg,ntsg)
	if c then
		local res=true
		if #sg>=3 or (not tg:IsExists(c73417207.chk2,1,c) and not ntg:IsExists(c73417207.chk2,1,c) 
