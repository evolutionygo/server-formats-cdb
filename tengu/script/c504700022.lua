...
function c504700022.initial_effect(c)
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc504700022(c504700022,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e1:SetCode(EVENT_BE_MATERIAL)
	e1:SetCondition(c504700022.spcon)
	e1:SetTarget(c504700022.sptg)
	e1:SetOperation(c504700022.spop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetDescription(aux.Stringc504700022(c504700022,0))
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e2:SetRange(0xff)
	e2:SetCode(EVENT_MSET)
	e2:SetCondition(c504700022.spcon2)
	c:RegisterEffect(e2)
end
c504700022.listed_names={15653824}
function c504700022.spcon(e,tp,eg,ep,ev,re,r,rp)
...
function c504700022.spcon2(e,tp,eg,ep,ev,re,r,rp)
...
function c504700022.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,e:GetLabel(),LOCATION_DECK)
end
function c504700022.spfilter(c,e,tp)
	return c:IsCode(15653824) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c504700022.spop(e,tp,eg,ep,ev,re,r,rp)
	local tp=e:GetLabel()
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tc=Duel.GetFirstMatchingCard(c504700022.spfilter,tp,LOCATION_DECK,0,nil,e,tp)
	if tc then
