...
c60406591.listed_names={60406592}
...
function c60406591.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ct>0 and (ct==1 or not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT))
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,c60406591+1,0,TYPES_TOKEN,100,100,1,RACE_PLANT,ATTRIBUTE_DARK) end
...
function c60406591.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if (ct>1 and Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT))
		...
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,c60406591+1,0,TYPES_TOKEN,100,100,1,RACE_PLANT,ATTRIBUTE_DARK) then return end
	for i=1,ct do
		local token=Duel.CreateToken(tp,c60406591+1)
