...
function c82670878.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc82670878(c82670878,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_CUSTOM+c82670878)
	...
	e2:SetDescription(aux.Stringc82670878(c82670878,1))
...
function c82670878.check(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetAttackTarget()==nil then
		...
		if s[1-tc:GetControler()]==1 then
			s[2]=tc
			tc:RegisterFlagEffect(c82670878,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		elseif s[1-tc:GetControler()]==2 then
			Duel.RaiseEvent(tc,EVENT_CUSTOM+c82670878,e,0,0,0,0)
		end
	end
end
function c82670878.check2(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	if tc:GetFlagEffect(c82670878)~=0 and Duel.GetAttackTarget()~=nil then
		s[1-tc:GetControler()]=s[1-tc:GetControler()]-1
	end
end
...
function c82670878.adop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFaceup() and c:IsRelateToEffect(e) then
		if tc and tc:GetFlagEffect(c82670878) then
			local e1=Effect.CreateEffect(c)
