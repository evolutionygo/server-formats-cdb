...
--Bubble Crash (GOAT)
--Cannot send itself
function c504700100.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE)
	e1:SetCondition(c504700100.condition)
	e1:SetOperation(c504700100.activate)
	c:RegisterEffect(e1)
end
function c504700100.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,0xe,0)>=6 or Duel.GetFieldGroupCount(tp,0,0xe)>=6
end
function c504700100.activate(e,tp,eg,ep,ev,re,r,rp)
