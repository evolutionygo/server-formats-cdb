...
function c511000817.initial_effect(c)
	c:AddSetcodesRule(c511000817,false,0x601)
	--dark synchro summon
	c:EnableReviveLimit()
	Synchro.AddDarkSynchroProcedure(c,aux.NonTuner(nil),nil,0)
	--spsummon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc511000817(c511000817,1))
	...
	e15:SetDescription(aux.Stringc511000817(c511000817,2))
...
c511000817.listed_series={0xc2}
function c511000817.spfilter(c,e,tp,ct)
...
function c511000817.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c511000817.spfilter,tp,LOCATION_EXTRA,0,ct,ct,nil,e,tp,ct)
	if #g==ct then
...
function c511000817.chkop(e,tp,eg,ep,ev,re,r,rp)
	for i=0,1 do
		if eg:IsExists(c511000817.cfilter,1,nil,i) then
			Duel.RegisterFlagEffect(i,c511000817,RESET_PHASE+PHASE_END,0,1)
		end
	end
end
function c511000817.setlimit(e,c,tp)
	return not c:IsLocation(LOCATION_ONFIELD) and Duel.GetFlagEffect(tp,c511000817)>0
end
function c511000817.sumlimit(e,c,sump,sumtype,sumpos,targetp)
	return sumpos&POS_FACEDOWN==POS_FACEDOWN and not c:IsLocation(LOCATION_ONFIELD) and Duel.GetFlagEffect(sump,c511000817)>0
end
