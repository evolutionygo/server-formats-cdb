...
function c64910482.initial_effect(c)
	...
	e3:SetLabel(c64910482)
	e3:SetValue(c64910482.synval)
	c:RegisterEffect(e3)
end
c64910482.listed_series={0x27}
...
function c64910482.chk2(c)
	...
	for i=1,#te do
		local e=te[i]
		if e:GetLabel()==c64910482 then return true end
	end
	return false
end
...
function c64910482.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsReason(REASON_DESTROY) then
		...
		e1:SetDescription(aux.Stringc64910482(c64910482,0))
...
function c64910482.filter(c)
	return c:IsSetCard(0x27) and c:GetCode()~=c64910482 and c:IsMonster() and c:IsAbleToHand()
end
