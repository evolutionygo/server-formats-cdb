...
function c511000308.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511000308(c511000308,0))
...
c511000308.listed_names={17132130,83965310,99357565}
...
function c511000308.op(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if tc:GetFlagEffect(c511000308)==0 then
			local e2=Effect.CreateEffect(tc)
			e2:SetDescription(aux.Stringc511000308(64382841,3))
			...
			tc:RegisterFlagEffect(c511000308,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		end
		tc=g1:GetNext()
	end
end
function c511000308.dogmacon(e,c)
	...
	local d3=g:FilterCount(Card.IsHasEffect,nil,c511000308)
...
function c511000308.dogmaop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local sg2=g:FilterSelect(tp,Card.IsHasEffect,1,1,sg1:GetFirst(),c511000308)
	sg1:Merge(sg2)
...
function c511000308.plasmacon(e,c)
	if c==nil then return true end
	local g=Duel.GetReleaseGroup(c:GetControler())
	local d2=g:FilterCount(Card.IsHasEffect,nil,c511000308)
	local d3=g:FilterCount(Card.IsHasEffect,nil,c511000308+1)
...
function c511000308.plasmaop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetReleaseGroup(tp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local sg1=g:FilterSelect(tp,Card.IsHasEffect,1,1,nil,c511000308)
	if not sg1:GetFirst():IsHasEffect(c511000308+1) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
