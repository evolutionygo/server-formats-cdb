...
function c511002641.vala(e,c)
	...
	return ca:GetFlagEffect(c511002641)>0 
		and (c:GetLevel()>ca:GetLevel()-ca:GetFlagEffect(c511002641) or c:IsFacedown())
end
function c511002641.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if d then
		...
		local lv=d:GetLevel()
		while i<lv do
			e:GetHandler():RegisterFlagEffect(c511002641,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_BATTLE,0,0)
			i=i+1
		end
	end
end
