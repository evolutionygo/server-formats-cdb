...
function c46291010.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc46291010(c46291010,0))
...
function c46291010.rmfilter(c)
	return c:IsRace(RACE_PSYCHIC) and c:IsAbleToRemove()
end
function c46291010.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c46291010.rmfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,tp,LOCATION_DECK)
end
function c46291010.rmop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c46291010.rmfilter,tp,LOCATION_DECK,0,1,1,nil)
	local tc=g:GetFirst()
