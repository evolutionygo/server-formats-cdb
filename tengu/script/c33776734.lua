...
function c33776734.initial_effect(c)
	c:SetUniqueOnField(1,0,c33776734)
	--special summon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc33776734(c33776734,0))
...
c33776734.listed_names={c33776734} --not standard, but required for Level Up!
c33776734.LVnum=9
c33776734.LVset=0x10a4
function c33776734.checkop1(e,tp,eg,ep,ev,re,r,rp)
	if re and re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_SPELL) then
		re:GetHandler():RegisterFlagEffect(c33776734,RESET_EVENT+RESETS_STANDARD,0,1)
	end
end
function c33776734.checkop2(e,tp,eg,ep,ev,re,r,rp)
	if re and re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_SPELL) then
		re:GetHandler():ResetFlagEffect(c33776734)
	end
end
...
function c33776734.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,c33776734)==0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
	Duel.RegisterFlagEffect(tp,c33776734,RESET_CHAIN,0,0)
end
...
function c33776734.rmtarget(e,c)
	return c:IsFaceup() and c:GetFlagEffect(c33776734)>0 and Duel.IsPlayerCanRemove(e:GetHandlerPlayer(),c)
end
