...
function c100000056.initial_effect(c)
	...
	e4:SetDescription(aux.Stringc100000056(c100000056,0))
	e4:SetCategory(CATEGORY_EQUIP)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCountLimit(1)
	e4:SetCondition(c100000056.eqcon)
	e4:SetTarget(c100000056.eqtg)
	e4:SetOperation(c100000056.eqop)
	c:RegisterEffect(e4)
	aux.AddEREquipLimit(c,c100000056.eqcon,function(ec,_,tp) return ec:IsControler(1-tp) and ec:IsFaceup() and ec:IsType(TYPE_SYNCHRO) end,c100000056.equipop,e4)
end
c100000056.listed_series={0x3013}
function c100000056.filter(c)
	return c:IsFaceup() and (c:IsWisel() or c:IsGranel() or c:IsSkiel())
end
function c100000056.val(e,c)
	return Duel.GetMatchingGroup(c100000056.filter,c:GetControler(),LOCATION_MZONE,0,c):GetSum(Card.GetAttack)
end
function c100000056.effval(e,re)
	return re:GetHandlerPlayer()~=e:GetHandlerPlayer()
end
function c100000056.eqlfilter(c)
	return c:GetFlagEffect(c100000056)~=0
end
function c100000056.eqcon(e,tp,eg,ep,ev,re,r,rp)
	local g=e:GetHandler():GetEquipGroup():Filter(c100000056.eqlfilter,nil)
	return #g==0
end
...
function c100000056.equipop(c,e,tp,tc)
	...
	if not c:EquipByEffectAndLimitRegister(e,tp,tc,c100000056) then return end
	if atk>0 then
		local e1=Effect.CreateEffect(c)
