...
function c511600382.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c511600382.target)
	e1:SetOperation(c511600382.activate)
	c:RegisterEffect(e1)
...
c511600382.listed_series={0x1e}
function c511600382.spfilter(c,e,tp,c511600382)
	return c:IsCode(c511600382) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
function c511600382.target(e,tp,eg,ep,ev,re,r,rp,chk)
		...
		for c in aux.Next(g) do
			...
			for _,v in ipairs(c.listed_names) do
				...
				if tok:IsMonster() then
					local tempg=Duel.GetMatchingGroup(c511600382.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil,e,tp,v)
...
function c511600382.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.FaceupFilter(Card.IsSetCard,0x1e),tp,LOCATION_MZONE,LOCATION_MZONE,nil)
...
function c511600382.rescon(sg,e,tp,mg)
