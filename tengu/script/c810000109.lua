...
function c810000109.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCode2N(c,true,true,aux.FilterBoolFunction(Card.IsSetCard,0xad),3)
	--special summon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc810000109(c810000109,0))
	...
	e3:SetDescription(aux.Stringc810000109(52085072,0))
...
function c810000109.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
		tc:RegisterFlagEffect(c810000109,RESET_EVENT+RESETS_STANDARD,0,0)
...
function c810000109.atkfilter(c)
	return c:GetFlagEffect(c810000109)>0
end
function c810000109.accon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local bt=Duel.GetAttacker()
	if e:GetHandler():GetFlagEffect(c810000109+1)>0 then return end
...
function c810000109.acop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	c:RegisterFlagEffect(c810000109+1,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_BATTLE,0,1)
	local e1=Effect.CreateEffect(c)
