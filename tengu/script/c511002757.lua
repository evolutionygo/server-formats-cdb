...
function c511002757.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local fc511002757=e:GetFieldID()
	...
	if tc1 then
		...
		if Duel.SpecialSummonStep(tc1,0,tp,tp,false,false,pos) then
			tc1:RegisterFlagEffect(c511002757,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_BATTLE,0,1,fc511002757)
		end
	end
	if Duel.GetLocationCountFromEx(1-tp,1-tp,nil,TYPE_XYZ)>0 then
		...
		if tc2 and Duel.SpecialSummonStep(tc2,0,1-tp,1-tp,false,false,pos) then
			tc2:RegisterFlagEffect(c511002757,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_BATTLE,0,1,fc511002757)
		end
	end
	...
	e1:SetCountLimit(1)
	e1:SetLabel(fc511002757)
	e1:SetLabelObject(sg)
	e1:SetCondition(c511002757.descon)
	e1:SetOperation(c511002757.desop)
	Duel.RegisterEffect(e1,tp)
end
function c511002757.desfilter(c,fc511002757)
	return c:GetFlagEffectLabel(c511002757)==fc511002757
end
