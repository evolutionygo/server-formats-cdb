...
function c511015115.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511015115(c511015115,0))
...
function c511015115.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return #g>1 and Duel.IsExistingMatchingCard(c511015115.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,g) end
	g:KeepAlive()
	e:SetLabelObject(g)
	local tc=Duel.GetFirstMatchingCard(c511015115.filter,tp,LOCATION_EXTRA,0,nil,e,tp,g)
	Duel.Overlay(tc,g)
end
...
function c511015115.disop(e,tp,eg,ep,ev,re,r,rp)
	Duel.RaiseEvent(re:GetHandler(),c511015115,e,REASON_EFFECT,tp,tp,0)
end
function c511015115.resetop(e,tp,eg,ep,ev,re,r,rp)
	local g=e:GetLabelObject()
	Duel.SendtoGrave(g,REASON_RULE)
end
function c511015115.filter(c,e,tp,mg)
...
function c511015115.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,c511015115.filter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,og):GetFirst()
	if tc then
		Duel.Overlay(tc,og)
		Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)
		tc:CompleteProcedure()
		c:SetCardTarget(tc)
		c:RegisterFlagEffect(c511015115,RESET_EVENT+RESETS_STANDARD,0,0)
	else
		Duel.SendtoGrave(g,REASON_RULE)
	end
end
function c511015115.regop2(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.RaiseSingleEvent(e:GetHandler(),c511015115+1,e,REASON_EFFECT,tp,tp,0)
	if g and #g>0 then
		g:KeepAlive()
		e:SetLabelObject(g)
	end
...
function c511015115.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetHandler():IsDisabled() and e:GetHandler():GetFlagEffect(c511015115)>0 then
		Duel.RaiseEvent(e:GetHandler(),c511015115+1,e,REASON_EFFECT,tp,tp,0)
		e:GetHandler():ResetFlagEffect(c511015115)
	end
end
