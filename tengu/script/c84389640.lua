...
function c84389640.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc84389640(c84389640,0))
...
function c84389640.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return e:GetHandler():GetFlagEffect(c84389640)==0 and Duel.CheckLPCost(tp,100)
	...
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringc84389640(c84389640,1))
	local pay=Duel.AnnounceNumber(tp,table.unpack(t))
	Duel.PayLPCost(tp,pay)
	e:SetLabel(-pay)
	e:GetHandler():RegisterFlagEffect(c84389640,RESET_PHASE+PHASE_DAMAGE,0,1)
end
