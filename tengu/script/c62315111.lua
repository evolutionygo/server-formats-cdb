...
function c62315111.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc62315111(c62315111,0))
...
c62315111.counter_list={COUNTER_A}
function c62315111.regop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local bc=c:GetBattleTarget()
	if bc and bc:GetCounter(COUNTER_A)>0 then
		c:RegisterFlagEffect(c62315111,RESET_PHASE+PHASE_DAMAGE,0,1)
	end
end
function c62315111.atcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return aux.bdcon(e,tp,eg,ep,ev,re,r,rp) and c:CanChainAttack() and c:GetFlagEffect(c62315111)~=0
end
