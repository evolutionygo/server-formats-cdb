...
function c511001433.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,c511001433,0,0x21,0,0,1,RACE_FIEND,ATTRIBUTE_LIGHT) end
...
function c511001433.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,c511001433,0,0x21,0,0,1,RACE_FIEND,ATTRIBUTE_LIGHT) then return end
	c:AddMonsterAttribute(TYPE_EFFECT)
	Duel.SpecialSummon(c,0,tp,tp,true,false,POS_FACEUP_ATTACK)
	c:AddMonsterAttributeComplete()
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc511001433(27062594,0))
