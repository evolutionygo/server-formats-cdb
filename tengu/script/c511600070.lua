...
function c511600070.initial_effect(c)
	--Xyz Summon
	c:EnableReviveLimit()
	aux.AddXyzProcedure(c,c511600070.xyzfilter,nil,3)
	--This card's Special Summon cannot be prevented or negated
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(c511600070)
	...
	e4:SetCondition(function(e) return e:GetHandler():HasFlagEffect(c511600070+1) end)
	e4:SetValue(function(e,c) return c:GetFlagEffectLabel(c511600070+1)*500 end)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	...
	aux.GlobalCheck(s,function()
		...
		Duel.Overlay=function(c,g,grave)
			...
			local label=c:GetFlagEffectLabel(c511600070+1)
			if not label then
				c:RegisterFlagEffect(c511600070+1,RESET_EVENT|RESETS_STANDARD&~RESET_TOFIELD,0,1,rank)
			else
				c:SetFlagEffectLabel(c511600070+1,label+rank)
			end
		...
		Card.RegisterEffect=function(c,e,...)
			if e:GetCode()==EFFECT_CANNOT_SPECIAL_SUMMON then
				...
				e:SetTarget(c511600070.splimit(oldTg))
			end
			return oldcf(c,e,...)
		end
...
c511600070.aux.xyz_number=0
c511600070.listed_series={SET_NUMBER,SET_NUMBER_S}
function c511600070.splimit(target)
	return function (e,c,...)
		return not c:IsHasEffect(c511600070) and (not target or target(e,c,...))
	end
end
function c511600070.xyzfilter(c,xyz,sumtype,tp)
