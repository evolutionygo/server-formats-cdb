...
function c513000004.initial_effect(c)
	c:AddSetcodesRule(c513000004,false,0x601)
	--dark synchro summon
	c:EnableReviveLimit()
	...
	e3:SetDescription(aux.Stringc513000004(c513000004,1))
...
function c513000004.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c513000004.filter,tp,LOCATION_GRAVE,0,nil)
	g:Remove(c513000004.codefilterchk,nil,e:GetHandler())
	if c:IsFacedown() or #g<=0 then return end
	repeat
		local tc=g:GetFirst()
		local code=tc:GetOriginalCode()
		local cc513000004=c:CopyEffect(code,RESET_EVENT+RESETS_STANDARD,1)
		c:RegisterFlagEffect(code,RESET_EVENT+RESETS_STANDARD,0,0)
		local e0=Effect.CreateEffect(c)
		e0:SetCode(c513000004)
		e0:SetLabel(code)
