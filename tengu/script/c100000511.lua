...
--updated by Larry126
local s,c100000511,alias=GetID()
function c100000511.initial_effect(c)
	alias=c:GetOriginalCodeRule()
	--Place in the Spell/Trap Zone
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc100000511(alias,0))
	...
	e2:SetDescription(aux.Stringc100000511(alias,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_SZONE)
	e2:SetHintTiming(0,TIMING_MAIN_END)
	e2:SetCondition(c100000511.spcon)
	...
	e3:SetDescription(aux.Stringc100000511(alias,2))
	e3:SetCategory(CATEGORY_ATKCHANGE)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	e3:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_CARD_TARGET)
	e3:SetCondition(c100000511.atkcon)
	e3:SetTarget(c100000511.atktg)
	e3:SetOperation(c100000511.atkop)
	c:RegisterEffect(e3)
end
c100000511.listed_series={0x5008}
...
function c100000511.spcon(e,tp,eg,ep,ev,re,r,rp)
...
function c100000511.atkcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_SZONE)
end
function c100000511.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and chkc:IsFaceup() end
	if chk==0 then return Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,Card.IsFaceup,tp,0,LOCATION_MZONE,1,1,nil)
end
function c100000511.atkop(e,tp,eg,ep,ev,re,r,rp)
