...
function c170000152.initial_effect(c)
	...
	e1:SetCondition(c170000152.condition)
...
function c170000152.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentChain()==0 and Duel.IsMainPhase() and Duel.GetTurnPlayer()==tp
end
function c170000152.filter1(c,e,tp)
	return Duel.IsExistingMatchingCard(c170000152.filter2,tp,LOCATION_EXTRA,0,1,nil,c:GetCode(),e,tp,c)
end
function c170000152.filter2(c,code,e,tp,mc)
...
function c170000152.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		local sc=Duel.SelectMatchingCard(tp,c170000152.filter2,tp,LOCATION_EXTRA,0,1,1,nil,tc:GetCode(),e,tp,tc):GetFirst()
		if sc then
			sc:SetMaterial(g)
