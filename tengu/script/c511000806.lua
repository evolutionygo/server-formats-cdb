...
function c511000806.chk2(c)
	...
	for i=1,#te do
		local e=te[i]
		if e:GetLabel()==c511000806 then return true end
	end
	return false
end
function c511000806.synchktg(e,c,sg,tg,ntg,tsg,ntsg)
	if c then
		local res=true
		if #sg>=2 or (not tg:IsExists(c511000806.chk2,1,c) and not ntg:IsExists(c511000806.chk2,1,c) 
...
function c511000806.synopfilter(c)
	...
	for i=1,#te do
		local e=te[i]
		if e:GetLabel()~=c511000806 then return false end
	end
	return true
end
