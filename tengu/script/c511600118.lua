	...
	for ec in aux.Next(eg) do
		if ec:IsPreviousLocation(LOCATION_MZONE) then
			if ec:IsPreviousControler(0) then
				Duel.RegisterFlagEffect(0,c511600118,RESET_PHASE+PHASE_END,0,1)
			else
				Duel.RegisterFlagEffect(1,c511600118,RESET_PHASE+PHASE_END,0,1)
			end
		end
	end
end
function c511600118.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFlagEffect(tp,c511600118)>0
end
function c511600118.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)>=Duel.GetFlagEffect(tp,c511600118) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,0,tp,LOCATION_DECK)
end
function c511600118.operation(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)==0 then return end
	local ct=Duel.GetFlagEffect(tp,c511600118)
