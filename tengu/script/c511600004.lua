...
function c511600004.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.Remove(tc,POS_FACEDOWN,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_REMOVED)
		...
		local tc511600004=Duel.GetTurnCount()
		local e1=Effect.CreateEffect(c)
		...
		e1:SetCondition(function()
			return Duel.IsTurnPlayer(1-tp) and Duel.GetTurnCount()~=tc511600004
		end)
		e1:SetOperation(c511600004.thop)
		e1:SetReset(RESET_EVENT|RESETS_STANDARD|RESET_PHASE|PHASE_STANDBY,8)
		tc:RegisterEffect(e1)
		if Duel.IsTurnPlayer(1-tp) then c511600004.thop(e1) end
		local descnum=tp==c:GetOwner() and 0 or 1
		local e2=Effect.CreateEffect(c)
		e2:SetDescription(aux.Stringc511600004(c511600004,descnum))
		...
		e2:SetOperation(c511600004.reset)
