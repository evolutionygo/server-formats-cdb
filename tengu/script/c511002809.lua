...
function c511002809.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCode2(c,true,true,aux.FilterBoolFunctionEx(Card.IsSetCard,0x10f2),aux.FilterBoolFunctionEx(Card.IsRace,RACE_SPELLCASTER))
	--multi attack
...
function c511002809.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if (flag&0x7)~=0 then
		...
		elseif flag==0x2 then
			e1:SetDescription(aux.Stringc511002809(1516510,1))
			e1:SetValue(2)
		else
			e1:SetDescription(aux.Stringc511002809(1516510,0))
			e1:SetValue(1)
		end
