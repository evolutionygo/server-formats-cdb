...
function c10925955.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if sel==1 then
		ac=Duel.SelectOption(tp,aux.Stringc10925955(c10925955,0))
	elseif sel==2 then
		ac=Duel.SelectOption(tp,aux.Stringc10925955(c10925955,1))+1
	elseif Duel.IsExistingMatchingCard(c10925955.cfilter,tp,LOCATION_MZONE,0,1,nil,true) then
		ac=Duel.SelectOption(tp,aux.Stringc10925955(c10925955,0),aux.Stringc10925955(c10925955,1),aux.Stringc10925955(c10925955,2))
	else
		ac=Duel.SelectOption(tp,aux.Stringc10925955(c10925955,0),aux.Stringc10925955(c10925955,1))
	end
	e:SetLabel(ac)
end
function c10925955.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ac==1 or ac==2 then
		if Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)>1 and Duel.GetFieldGroupCount(tp,0,LOCATION_DECK)>1 then
			local st=Duel.SelectOption(tp,aux.Stringc10925955(c10925955,3),aux.Stringc10925955(c10925955,4))
