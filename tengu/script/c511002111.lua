...
function c511002111.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511002111(63746411,0))
	e2:SetCategory(CATEGORY_DISABLE)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetCode(EVENT_CHAINING)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCondition(c511002111.discon)
	e2:SetCost(aux.dxmcostgen(1,1,nil))
	e2:SetTarget(c511002111.distg)
	e2:SetOperation(c511002111.disop)
	c:RegisterEffect(e2,false,REGISTER_FLAG_DETACH_XMAT)
	--Destroy monster targeted by previous effect when it declares an attack
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc511002111(50491121,0))
	e3:SetCategory(CATEGORY_DESTROY+CATEGORY_DAMAGE)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e3:SetCode(EVENT_ATTACK_ANNOUNCE)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c511002111.descon)
	e3:SetTarget(c511002111.destg)
	e3:SetOperation(c511002111.desop)
	c:RegisterEffect(e3)
end
c511002111.listed_series={SET_NUMBER}
c511002111.aux.xyz_number=106
function c511002111.discon(e,tp,eg,ep,ev,re,r,rp)
	return rp~=tp and re:IsMonsterEffect() and Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_LOCATION)==LOCATION_MZONE
end
function c511002111.distg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c511002111.disop(e,tp,eg,ep,ev,re,r,rp)
