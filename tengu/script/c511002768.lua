...
function c511002768.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if tc:IsType(TYPE_XYZ) and tc:IsReason(REASON_DESTROY) and tc:IsReason(REASON_BATTLE) then
			tc:RegisterFlagEffect(c511002768,RESET_EVENT+RESETS_STANDARD,0,1)
		end
		tc=eg:GetNext()
	end
end
function c511002768.filter1(c,e,tp)
	...
	return #pg<=1 and c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:GetFlagEffect(c511002768)~=0
