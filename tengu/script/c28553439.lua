...
function c28553439.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc28553439(c28553439,0))
...
function c28553439.rfilter(c,tp)
	return c:IsReleasableByEffect() and Duel.GetMZoneCount(tp,c)>0
end
function c28553439.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c28553439.rfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c28553439.rfilter,tp,LOCATION_MZONE,0,1,nil,tp)
		and Duel.IsExistingMatchingCard(c28553439.filter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local g=Duel.SelectTarget(tp,c28553439.rfilter,tp,LOCATION_MZONE,0,1,1,nil,tp)
...
function c28553439.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsControler(tp) and Duel.Release(tc,REASON_EFFECT)>0
		...
		if #sg>0 and Duel.SpecialSummon(sg,0,tp,tp,false,false,POS_FACEUP)>0 then
			...
			if #dg>0 and Duel.SelectYesNo(tp,aux.Stringc28553439(c28553439,1)) then
