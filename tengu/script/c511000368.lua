...
function c511000368.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511000368(c511000368,0))
	e2:SetCategory(CATEGORY_POSITION)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetTarget(c511000368.postg)
	e2:SetOperation(c511000368.posop)
	c:RegisterEffect(e2)
	...
	e4:SetOperation(c511000368.regop)
	c:RegisterEffect(e4)
	...
	e5:SetDescription(aux.Stringc511000368(c511000368,1))
	e5:SetCategory(CATEGORY_REMOVE)
	e5:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e5:SetCode(EVENT_PHASE+PHASE_STANDBY)
	e5:SetRange(LOCATION_MZONE)
	e5:SetCountLimit(1)
	e5:SetCondition(function(e) return e:GetHandler():HasFlagEffect(c511000368) end)
	e5:SetTarget(c511000368.rmtg)
	e5:SetOperation(c511000368.rmop)
	c:RegisterEffect(e5)
	--Special Summon this card from your GY
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringc511000368(c511000368,2))
	e6:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_ATKCHANGE)
	e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e6:SetCode(EVENT_TO_GRAVE)
	e6:SetCondition(c511000368.spcon)
	e6:SetTarget(c511000368.sptg)
	e6:SetOperation(c511000368.spop)
	c:RegisterEffect(e6)
end
c511000368.aux.xyz_number=92
function c511000368.posfilter(c,e)
	return c:IsAttackPos() and c:IsCanBeEffectTarget(e)
end
function c511000368.postg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local g=Duel.GetMatchingGroup(c511000368.posfilter,tp,0,LOCATION_MZONE,nil,e)
	if #g>0 then
...
function c511000368.damval(e,re,val,r,rp,rc)
	local c=e:GetHandler()
...
function c511000368.regop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	c:RegisterFlagEffect(c511000368,RESET_EVENT|RESETS_STANDARD,0,1)
	...
	e1:SetOperation(c511000368.trackop)
	e1:SetReset(RESET_EVENT|RESETS_STANDARD)
	c:RegisterEffect(e1)
end
function c511000368.trackop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=eg:Filter(Card.IsControler,nil,1-tp)
	local reset_ct=Duel.GetCurrentPhase()<=PHASE_STANDBY and 2 or 1
	for tc in g:Iter() do
		c:CreateRelation(tc,RESET_EVENT|RESETS_STANDARD)
		tc:RegisterFlagEffect(c511000368+1,RESET_EVENT|RESETS_STANDARD|RESET_PHASE|PHASE_STANDBY,0,reset_ct,Duel.GetTurnCount()+1)
	end
end
function c511000368.rmfilter(c,hc,turn_ct)
	return hc:IsRelateToCard(c) and c:HasFlagEffect(c511000368+1) and c:GetFlagEffectLabel(c511000368+1)==turn_ct
end
function c511000368.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local g=Duel.GetMatchingGroup(c511000368.rmfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil,e:GetHandler(),Duel.GetTurnCount())
	Duel.SetTargetCard(g)
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,#g,tp,0)
end
function c511000368.rmop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetTargetCards(e):Match(Card.HasFlagEffect,nil,c511000368+1)
	if #g>0 then
...
function c511000368.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return not c:HasFlagEffect(c511000368+2)
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
	c:RegisterFlagEffect(c511000368+2,(RESET_EVENT|RESETS_STANDARD|RESET_MSCHANGE)&~(RESET_TOFIELD|RESET_TOGRAVE|RESET_LEAVE|RESET_TURN_SET),EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringc511000368(c511000368,3))
