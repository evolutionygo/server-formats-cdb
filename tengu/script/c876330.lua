...
function c876330.initial_effect(c)
	...
	e1:SetCondition(c876330.spcon1)
	e1:SetTarget(c876330.sptg1)
	e1:SetOperation(c876330.spop1)
	c:RegisterEffect(e1)
	--equip
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc876330(c876330,0))
	...
	c:RegisterEffect(e2)
	aux.AddEREquipLimit(c,nil,c876330.eqval,Card.EquipByEffectAndLimitRegister,e2)
	local e3=e2:Clone()
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	c:RegisterEffect(e3)
	aux.AddEREquipLimit(c,nil,c876330.eqval,Card.EquipByEffectAndLimitRegister,e3)
end
...
function c876330.spcon1(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local rg=Duel.GetMatchingGroup(c876330.spfilter,tp,LOCATION_MZONE,0,nil,ft)
	return ft>-1 and #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,1,nil,0)
end
function c876330.sptg1(e,tp,eg,ep,ev,re,r,rp,c)
	local c=e:GetHandler()
	local g=nil
	local rg=Duel.GetMatchingGroup(c876330.spfilter,tp,LOCATION_MZONE,0,nil,Duel.GetLocationCount(tp,LOCATION_MZONE))
...
function c876330.spop1(e,tp,eg,ep,ev,re,r,rp,c)
...
function c876330.filter(c)
	return c:IsSetCard(0x29) and c:IsRace(RACE_DRAGON) and not c:IsForbc876330den()
end
