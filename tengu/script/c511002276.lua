...
function c511002276.atop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsAttackCostPac511002276()~=2 and c:IsLocation(LOCATION_MZONE) then
		if Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)==0 or not da then
			Duel.AttackCostPac511002276()
			return
		end
		...
		if c:GetEffectCount(EFFECT_DIRECT_ATTACK)>1 then
			if Duel.CheckReleaseGroup(tp,nil,1,c) and Duel.SelectYesNo(tp,500) then
				...
				local tc=Duel.GetReleaseGroup(tp):Filter(aux.TRUE,c):SelectUnselect(Group.CreateGroup(),tp,Duel.IsAttackCostPac511002276()==0, Duel.IsAttackCostPac511002276()==0)
				if tc then
