...
function c504700140.filter(c)
	return c:GetSequence()<5 and not c:IsType(TYPE_TOKEN+TYPE_LINK)
end
function c504700140.spfilter(c,e,tp)
	return c:IsSpellTrap() and Duel.IsPlayerCanSpecialSummonMonster(tp,c:GetCode(),nil,0x11,0,0,0,0,0,POS_FACEDOWN)
end
function c504700140.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c504700140.filter,tp,LOCATION_MZONE,0,1,nil)
...
function c504700140.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c504700140.spfilter,tp,LOCATION_DECK,0,nil,e,tp)
	if #g<2 then Duel.GoatConfirm(tp,LOCATION_DECK) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local tc=Duel.SelectMatchingCard(tp,c504700140.filter,tp,LOCATION_MZONE,0,1,1,nil):GetFirst()
	...
	local fc504700140=e:GetHandler():GetFieldID()
	for tg in aux.Next(sg) do
		...
		tg:RegisterFlagEffect(c504700140,RESET_EVENT+0x47c0000+RESET_PHASE+PHASE_BATTLE,0,1,fc504700140)
	...
	de:SetCountLimit(1)
	de:SetLabel(fc504700140)
	de:SetLabelObject(sg)
	de:SetOperation(c504700140.desop)
	Duel.RegisterEffect(de,tp)
end
function c504700140.desfilter(c,fc504700140)
	return c:GetFlagEffectLabel(c504700140)==fc504700140
end
function c504700140.desop(e,tp,eg,ep,ev,re,r,rp)
	local g=e:GetLabelObject()
	local fc504700140=e:GetLabel()
	local tg=g:Filter(c504700140.desfilter,nil,fc504700140)
	g:DeleteGroup()
	Duel.Destroy(tg,REASON_EFFECT)
	local tg2=tg:Filter(c504700140.desfilter,nil,fc504700140)
