...
function c51447164.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc51447164(c51447164,0))
	...
	e2:SetDescription(aux.Stringc51447164(c51447164,1))
	...
	e3:SetDescription(aux.Stringc51447164(c51447164,2))
	...
	c:RegisterEffect(e3)
	aux.DoubleSnareValc51447164ity(c,LOCATION_MZONE)
end
c51447164.listed_series={0x27}
c51447164.synchro_tuner_required=1
c51447164.synchro_nt_required=1
...
function c51447164.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.Remove(c,POS_FACEUP,REASON_EFFECT)~=0 then
		if Duel.GetCurrentPhase()==PHASE_STANDBY then
			c:RegisterFlagEffect(c51447164,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY,0,2,Duel.GetTurnCount())
		else
			c:RegisterFlagEffect(c51447164,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY,0,1)
		end
	end
end
function c51447164.spcon(e,tp,eg,ep,ev,re,r,rp)
	local label=e:GetHandler():GetFlagEffectLabel(c51447164)
	return label and label~=Duel.GetTurnCount()
end
function c51447164.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	e:GetHandler():ResetFlagEffect(c51447164)
