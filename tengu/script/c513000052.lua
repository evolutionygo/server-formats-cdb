...
function c513000052.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc513000052(c513000052,0))
	...
	e6:SetTarget(c513000052.reptg)
	e6:SetOperation(c513000052.repop)
	c:RegisterEffect(e6)
end
c513000052.listed_series={SET_TIMELORD}
c513000052.listed_names={72883039,8967776}
...
function c513000052.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c513000052.filter,tp,LOCATION_HDG,0,1,nil,e,tp) end
...
function c513000052.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c513000052.filter),tp,LOCATION_HDG,0,ft,ft,nil,e,tp)
...
function c513000052.damop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToBattle() and Duel.IsExistingMatchingCard(c513000052.repfilter,tp,LOCATION_MZONE,0,1,c)
...
function c513000052.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return not c:IsReason(REASON_REPLACE) and c:IsReason(REASON_EFFECT)
		and Duel.IsExistingMatchingCard(c513000052.repfilter,tp,LOCATION_MZONE,0,1,c) end
	if Duel.SelectEffectYesNo(tp,c,96) then return true
	else return false end
end
function c513000052.repop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESREPLACE)
	local g=Duel.SelectMatchingCard(tp,c513000052.repfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
	Duel.Remove(g,POS_FACEUP,REASON_EFFECT)
end