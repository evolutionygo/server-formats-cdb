...
function c511016005.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511016005(76647978,0))
...
function c511016005.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local mg=Duel.GetMatchingGroup(c511016005.mfilter,tp,LOCATION_MZONE,0,nil)
...
function c511016005.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(tc,SUMMON_TYPE_FUSION,tp,tp,false,false,POS_FACEUP) then
		...
		if #spg>1 then
			Duel.BreakEffect()
			local fc511016005=e:GetHandler():GetFieldID()
			...
			while spgc do
				...
				spgc:RegisterFlagEffect(51116005,RESET_PHASE+PHASE_END,0,1,fc511016005)
				spgc=spg:GetNext()
			end
			...
			if (con1 or con2) and Duel.SelectYesNo(tp,65) then
				if con1 and con2 then
					op=Duel.SelectOption(tp,aux.Stringc511016005(1006081,0),aux.Stringc511016005(82744076,0))
				elseif con1 then
					Duel.SelectOption(tp,aux.Stringc511016005(1006081,0))
					op=0
				elseif con2 then
					Duel.SelectOption(tp,aux.Stringc511016005(82744076,0))
					op=1
				end
				if op==0 then
...
function c511016005.chkfilter(c,fc511016005)
	return c:GetFlagEffectLabel(51116005)==fc511016005 and c:GetFlagEffect(c511016005)>0
end
...
function c511016005.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #sg>0 and (r==REASON_SYNCHRO or r==REASON_XYZ) then
		local tc=sg:GetFirst()
		while tc do
			tc:RegisterFlagEffect(c511016005,RESET_PHASE+PHASE_END,0,0)
			tc=sg:GetNext()
		end
	end
end