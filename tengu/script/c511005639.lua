...
function c511005639.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc511005639(c511005639,0))
...
function c511005639.spctg(e,tp,eg,ep,ev,re,r,rp,c)
...
function c511005639.coinop(e,tp,eg,ep,ev,re,r,rp)
	...
	c511005639.arcanareg(c,Arcana.TossCoin(c,tp))
end
...
function c511005639.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Destroy(g,REASON_EFFECT)>0 and c:IsRelateToEffect(e) and c:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED) then
		c:RegisterFlagEffect(c511005639,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY+RESET_SELF_TURN,0,1)
		local e1=Effect.CreateEffect(c)
...
function c511005639.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_CARD,0,c511005639)
	if tc and tc:GetFlagEffect(c511005639)>0 and Duel.SpecialSummon(tc,0,tp,tp,true,false,POS_FACEUP)>0 then
