...
function c511002591.mtfilter(c)
	return c:IsFaceup() and not c:IsHasEffect(EFFECT_EXTRA_ATTACK) and c:GetFlagEffect(c511002591)==0
end
function c511002591.mttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	tc:RegisterFlagEffect(c511002591,RESET_CHAIN,0,1)
end
function c511002591.mtop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	local fc511002591=c:GetFieldID()
	if tc and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
		...
		tc:RegisterFlagEffect(51102591,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,fc511002591)
		--destroy
		local e2=Effect.CreateEffect(c)
...
function c511002591.desop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,c511002591)
	local tc=e:GetLabelObject()
