...
function c76922029.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc76922029(c76922029,0))
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_BATTLE_DAMAGE)
	e1:SetCondition(function(e,tp,eg,ep,ev,re,r,rp) return ep==1-tp end)
	e1:SetTarget(c76922029.efftg)
	e1:SetOperation(c76922029.effop)
	c:RegisterEffect(e1)
end
function c76922029.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)>0
	local b2=Duel.IsPlayerCanDiscardDeck(1-tp,2)
	if chk==0 then return b1 or b2 end
	local op=Duel.SelectEffect(tp,
		{b1,aux.Stringc76922029(c76922029,1)},
		{b2,aux.Stringc76922029(c76922029,2)})
	e:SetLabel(op)
	if op==1 then
...
function c76922029.effop(e,tp,eg,ep,ev,re,r,rp)
