...
function c513000055.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc513000055(c513000055,0))
	...
	e1:SetOperation(c513000055.activate)
	c:RegisterEffect(e1)
	...
	aux.GlobalCheck(s,function()
		...
		ge1:SetOperation(c513000055.checkop)
		Duel.RegisterEffect(ge1,0)
	end)
end
function c513000055.checkop(e,tp,eg,ep,ev,re,r,rp)
	if re:IsSpellEffect() and re:IsHasType(EFFECT_TYPE_ACTIVATE) then
		re:GetHandler():RegisterFlagEffect(c513000055+1,RESETS_STANDARD_PHASE_END&~(RESET_TOGRAVE|RESET_LEAVE),0,1)
	end
end
function c513000055.tgfilter(c,tp,szone_adjustment)
	if not c:IsSpell() or c:IsCode(c513000055) or not c:HasFlagEffect(c513000055+1) then return false end
	...
	if not Duel.IsChainSolving() then
		...
		local eff=c:IsHasEffect(c513000055)
...
function c513000055.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local szone_adjustment=c:IsLocation(LOCATION_SZONE) and 0 or 1
	if chk==0 then return Duel.IsExistingMatchingCard(c513000055.tgfilter,tp,0,LOCATION_GRAVE,1,c,tp,szone_adjustment) end
	local g=Duel.GetMatchingGroup(Card.HasFlagEffect,tp,0,LOCATION_GRAVE,c,c513000055+1)
	for tc in g:Iter() do
		...
		e1:SetCode(c513000055)
		e1:SetLabelObject(t)
		tc:RegisterEffect(e1)
	end
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_GRAVE,nil,1,1-tp,0)
end
function c513000055.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c513000055.tgfilter,tp,0,LOCATION_GRAVE,c,tp,0)
	if #g==0 then return end
	repeat
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EFFECT)
		local sc=g:Select(tp,1,1,nil):GetFirst()
		if sc then
			c513000055.place_and_activate(sc,e,tp,eg,ep,ev,re,r,rp)
		end
		g=Duel.GetMatchingGroup(c513000055.tgfilter,tp,0,LOCATION_GRAVE,c,tp,0)
	until #g==0 or not Duel.SelectEffectYesNo(tp,c)
end
function c513000055.place_and_activate(sc,e,tp,eg,ep,ev,re,r,rp)
	if not c513000055.tgfilter(sc,tp,0) then return end
	...
	local eff=sc:IsHasEffect(c513000055)
	local t=eff:GetLabelObject()
	eff:Reset()
