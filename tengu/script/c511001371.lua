...
function c511001371.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511001371(c511001371,0))
	e2:SetCategory(CATEGORY_DRAW)
	e2:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e2:SetCode(EVENT_BATTLE_DESTROYING)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e2:SetTarget(c511001371.drtg)
	e2:SetOperation(c511001371.drop)
	c:RegisterEffect(e2)
	--counter
	local e3=Effect.CreateEffect(c)
	e3:SetCategory(CATEGORY_COUNTER)
	e3:SetDescription(aux.Stringc511001371(c511001371,1))
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetCountLimit(1)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCost(aux.dxmcostgen(1,1,nil))
	e3:SetTarget(c511001371.cttg)
	e3:SetOperation(c511001371.ctop)
	--destroy
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc511001371(c511001371,2))
	e4:SetCategory(CATEGORY_DESTROY+CATEGORY_DAMAGE+CATEGORY_COUNTER)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCountLimit(1)
	e4:SetTarget(c511001371.destg)
	e4:SetOperation(c511001371.desop)
...
c511001371.listed_series={SET_NUMBER}
c511001371.listed_names={75433814}
c511001371.aux.xyz_number=40
