...
function c513000149.spfilter(c,tp)
...
function c513000149.sctg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_EXTRA) and c513000149.spfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingMatchingCard(c513000149.spfilter,tp,LOCATION_EXTRA,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c513000149.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,tp)
...
function c513000149.scop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetOperation(c513000149.regop)
	tc:RegisterEffect(e1)
	Duel.SynchroSummon(tp,tc,nil,mg)
end
function c513000149.regop(e,tp,eg,ep,ev,re,r,rp)
	local rc=e:GetOwner()
