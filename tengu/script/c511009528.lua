...
function c511009528.initial_effect(c)
	--fusion summon
	c:EnableReviveLimit()
	aux.AddFusionProcCode2N(c,true,true,aux.FilterBoolFunctionEx(Card.IsAttribute,ATTRIBUTE_DARK),2)
	--special summon
	...
	e3:SetDescription(aux.Stringc511009528(c511009528,1))
	...
	e5:SetDescription(aux.Stringc511009528(c511009528,2))
...
c511009528.listed_series={SET_SUPREME_KING_DRAGON}
...
function c511009528.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:CheckFusionMaterial() and eg:IsExists(c511009528.spfilter,1,nil,tp)
...
function c511009528.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.CheckReleaseGroup(tp,c511009528.costfilter,1,nil,tp,sg,c) and e:GetHandler():IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,true)
		and c:CheckFusionMaterial() and Duel.SelectEffectYesNo(tp,c,aux.Stringc511009528(c511009528,0)) then
		while #sg<2 do
...
function c511009528.cptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(Card.IsMonster,tp,LOCATION_GRAVE|LOCATION_MZONE,LOCATION_GRAVE|LOCATION_MZONE,1,nil)
		and e:GetHandler():GetFlagEffect(c511009528+1)==0 end
...
function c511009528.cpop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsRelateToEffect(e) and not tc:IsType(TYPE_TOKEN) then
		...
		if not tc:IsType(TYPE_TRAPMONSTER) then
			...
			c:RegisterFlagEffect(c511009528+1,RESET_EVENT|RESETS_STANDARD|RESET_PHASE|PHASE_END,0,0)
		end
	end
end
...
function c511009528.spfilter2(c,e,tp,ec)
...
function c511009528.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)
		and aux.CheckSummonGate(tp,2) and e:GetHandler():GetFlagEffect(c511009528)==0
		and Duel.IsExistingMatchingCard(c511009528.spfilter2,tp,LOCATION_EXTRA,0,2,nil,e,tp,e:GetHandler()) end
	e:GetHandler():RegisterFlagEffect(c511009528,RESET_CHAIN,0,0)
...
function c511009528.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c511009528.spfilter2,tp,LOCATION_EXTRA,0,nil,e,tp,e:GetHandler())
	if #g>=2 then
