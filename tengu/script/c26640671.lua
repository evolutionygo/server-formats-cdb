...
function c26640671.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc26640671(c26640671,0))
...
function c26640671.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if d and d:IsControler(1-tp) and e:GetHandler():IsHasCardTarget(a) then
		d:RegisterFlagEffect(c26640671,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	elseif d and a:IsControler(1-tp) and e:GetHandler():IsHasCardTarget(d) then
		a:RegisterFlagEffect(c26640671,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	end
end
function c26640671.pfilter(c)
	return c:GetFlagEffect(c26640671)~=0 and c:IsAttackPos()
end
function c26640671.posop(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(g) do
		tc:RegisterFlagEffect(c26640671+1,RESET_EVENT+RESETS_STANDARD,0,1)
	end
end
function c26640671.postg(e,c)
	return c:GetFlagEffect(c26640671+1)~=0
end