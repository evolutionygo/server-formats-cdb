...
function c511015114.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511015114(c511015114,0))
	...
	e2:SetDescription(aux.Stringc511015114(c511015114,1))
...
c511015114.listed_series={0x54,0x48}
...
function c511015114.xyzfilter(c,e,tp)
	...
	local mg=Duel.GetMatchingGroup(c511015114.filter,tp,LOCATION_MZONE,0,nil,c,tp)
	return mg:GetFirst() and Duel.GetMZoneCount(tp,mg)>0
end
function c511015114.xyztg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c511015114.xyzfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE)
end
function c511015114.xyzop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c511015114.xyzfilter,tp,LOCATION_GRAVE,0,nil,e,tp)
	if #g>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local tc=g:Select(tp,1,1,nil):GetFirst()
		local mg=Duel.GetMatchingGroup(c511015114.filter,tp,LOCATION_MZONE,0,nil,tc,tp)
		tc:SetMaterial(mg)
		Duel.Overlay(tc,mg)
