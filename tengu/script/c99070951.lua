...
function c99070951.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc99070951(c99070951,0))
	...
	e2:SetDescription(aux.Stringc99070951(c99070951,1))
...
function c99070951.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	tc:RegisterFlagEffect(c99070951,RESET_EVENT+RESETS_STANDARD,0,1)
	if c:IsLocation(LOCATION_MZONE) then
		c:RegisterFlagEffect(c99070951,RESET_EVENT+0x680000,0,1)
	end
	e:SetLabelObject(tc)
end
function c99070951.thcon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject():GetLabelObject()
	return e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and e:GetHandler():GetFlagEffect(c99070951)~=0
		and tc and tc:GetFlagEffect(c99070951)~=0
end
...
function c99070951.thop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject():GetLabelObject()
	if tc and tc:GetFlagEffect(c99070951)~=0 then
