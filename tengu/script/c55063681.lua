...
function c55063681.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc55063681(c55063681,0))
...
function c55063681.filter(c,ec)
	return c:IsFaceup() and ((ec==c and c:GetFlagEffect(c55063681)==0) or (ec~=c and not ec:IsHasCardTarget(c)))
end
...
function c55063681.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and c:IsFaceup() and c:IsRelateToEffect(e) then
		if tc~=c then
			...
			e1:SetReset(RESET_EVENT+RESETS_STANDARD)
			tc:RegisterEffect(e1)
			tc:RegisterFlagEffect(c55063681,RESET_EVENT+RESETS_STANDARD,0,1)
		end
	end
end
