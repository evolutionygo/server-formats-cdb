...
function c55794644.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc55794644(c55794644,0))
...
function c55794644.hspcon(e,c)
	...
	local rg=Duel.GetMatchingGroup(c55794644.spfilter,tp,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
	return aux.SelectUnselectGroup(rg,e,tp,1,1,aux.ChkfMMZ(1),0)
end
function c55794644.hsptg(e,tp,eg,ep,ev,re,r,rp,c)
	local rg=Duel.GetMatchingGroup(c55794644.spfilter,tp,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
...
function c55794644.condition(e,tp,eg,ep,ev,re,r,rp)
	if (Duel.IsExistingMatchingCard(c55794644.envfilter,e:GetHandlerPlayer(),LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) or Duel.IsEnvironment(CARD_SANCTUARY_SKY)) then return e:GetHandler():GetFlagEffect(c55794644)<2
	else return e:GetHandler():GetFlagEffect(c55794644)<1 end
end
...
function c55794644.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	e:GetHandler():RegisterFlagEffect(c55794644,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
end
