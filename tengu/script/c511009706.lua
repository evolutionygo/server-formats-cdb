...
function c511009706.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511009706(c511009706,0))
	...
	e3:SetDescription(aux.Stringc511009706(c511009706,1))
...
function c511009706.spfilter(c,e,tp,tc511009706)
	return c:GetTurnID()==tc511009706 and c:GetReason()&REASON_BATTLE~=0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511009706.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc511009706=Duel.GetTurnCount()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c511009706.spfilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,nil,e,tp,tc511009706) end
...
function c511009706.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c511009706.spfilter),tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil,e,tp,tc511009706)
	local tc=g:GetFirst()
	if tc then
		...
		if Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
			...
			tc:RegisterFlagEffect(c511009706,RESET_EVENT+RESETS_STANDARD,0,1)
...
function c511009706.descon(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:GetFlagEffect(c511009706)==0 then
		e:Reset()
		return false
	end
	return true
