...
c53291093.listed_series={0xc}
c53291093.counter_list={COUNTER_A}
...
function c53291093.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)~=0 
		...
		if #g>0 and Duel.SelectYesNo(tp,aux.Stringc53291093(c53291093,0)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
			local sg=g:Select(tp,1,1,nil)
			Duel.BreakEffect()
			Duel.SpecialSummon(sg,0,tp,tp,false,false,POS_FACEUP)
			local sc=sg:GetFirst()
			local fc53291093=e:GetHandler():GetFieldID()
			sc:RegisterFlagEffect(c53291093,RESET_EVENT+RESETS_STANDARD,0,1,fc53291093)
			local e1=Effect.CreateEffect(e:GetHandler())
			e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
			e1:SetCode(EVENT_PHASE+PHASE_END)
			e1:SetCountLimit(1)
			e1:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE)
			e1:SetLabel(fc53291093)
...
function c53291093.descon(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:GetFlagEffectLabel(c53291093)~=e:GetLabel() then
		e:Reset()
		return false
