...
function c140000131.initial_effect(c)
	...
	e2:SetTarget(c140000131.target)
	e2:SetOperation(c140000131.operation)
	c:RegisterEffect(e2)
	--Equip limit
...
c140000131.listed_names={71625222,46232525}
c140000131.roll_dice=true
	...
	if c:IsLocation(LOCATION_SZONE) and tc:IsLocation(LOCATION_MZONE) then
		if Duel.Equip(tp,c,tc) and not c:IsDisabled() then
			...
			for i=1,ct do
				...
				tc:RegisterFlagEffect(c140000131,RESET_EVENT+RESETS_REDIRECT-RESET_TEMP_REMOVE,0,1,m)
				g:RemoveCard(tc)
			end
			--Future Swing
			local e1=Effect.CreateEffect(c)
...
function c140000131.filter(c,rc,ct)
	return rc:GetCardTarget():IsContains(c) and c:GetFlagEffect(c140000131)>0
		and c:GetFlagEffectLabel(c140000131)==ct
end
function c140000131.act(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetLabel()==10 then
		...
		for tc in aux.Next(g) do
			if tc:GetFlagEffect(c140000131)>0 then
				c:SetCardTarget(tc)
				local ct=tc:GetFlagEffectLabel(c140000131)
				maxct=math.max(ct,maxct)
			end
		end
		if maxct==7 then
			c:RegisterFlagEffect(c140000131,RESET_EVENT+RESETS_STANDARD,0,1)
			e:SetLabel(7)
		else
			e:SetLabel(6)
		end
	elseif e:GetLabel()>0 then
		if c:GetFlagEffect(c140000131)>0 then
			c:SetTurnCounter(8-e:GetLabel())
			g=Duel.GetMatchingGroup(c140000131.filter,tp,LOCATION_REMOVED,LOCATION_REMOVED,nil,c,8-e:GetLabel())
		else
			c:SetTurnCounter(7-e:GetLabel())
			g=Duel.GetMatchingGroup(c140000131.filter,tp,LOCATION_REMOVED,LOCATION_REMOVED,nil,c,7-e:GetLabel())
		end
		if #g>0 then
