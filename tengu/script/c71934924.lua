...
function c71934924.atop(e,tp,eg,ep,ev,re,r,rp)
	if c71934924.check(Duel.GetAttacker(),tp) or c71934924.check(Duel.GetAttackTarget(),tp) then
		e:GetHandler():RegisterFlagEffect(c71934924,RESET_PHASE+PHASE_DAMAGE,0,1)
	end
end
function c71934924.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x3d) and c:GetFlagEffect(c71934924)==0
end
function c71934924.upop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:GetFlagEffect(c71934924)==0 then return end
	...
	for tc in aux.Next(g) do
		...
		tc:RegisterFlagEffect(c71934924,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	end
end