...
function c511005701.initial_effect(c)
	...
	e1:SetCondition(c511005701.nobattledmgcon)
	e1:SetValue(1)
	c:RegisterEffect(e1)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetCode(EVENT_DESTROYED)
	e2:SetOperation(c511005701.nodamop)
	c:RegisterEffect(e2)
	--Special Summon 1 card with the same name as this card from your Deck
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc511005701(c511005701,0))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_DESTROYED)
	e3:SetTarget(c511005701.sptg)
	e3:SetOperation(c511005701.spop)
	c:RegisterEffect(e3)
end
function c511005701.nobattledmgcon(e)
...
function c511005701.nodamop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local prev_ctrl=c:GetPreviousControler()
	--Its controller takes no damage this turn
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc511005701(c511005701,1))
...
function c511005701.spfilter(c,e,tp,code)
	return c:IsCode(code) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511005701.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c511005701.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,e:GetHandler():GetCode()) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c511005701.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	local g=Duel.SelectMatchingCard(tp,c511005701.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp,c:GetCode())
	if #g>0 then
