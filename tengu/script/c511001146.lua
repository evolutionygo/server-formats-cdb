...
function c511001146.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511001146(c511001146,0))
	...
	c:RegisterEffect(e1)
	aux.AddEREquipLimit(c,nil,c511001146.eqval,c511001146.equipop,e1)
	local e2=e1:Clone()
	e2:SetCode(EVENT_FLIP_SUMMON_SUCCESS)
	c:RegisterEffect(e2)
	aux.AddEREquipLimit(c,nil,c511001146.eqval,c511001146.equipop,e2)
	local e3=e1:Clone()
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	c:RegisterEffect(e3)
	aux.AddEREquipLimit(c,nil,c511001146.eqval,c511001146.equipop,e3)
	--Can attack directly
	...
	e5:SetCondition(c511001146.atkcon)
	e5:SetOperation(c511001146.atkop)
	c:RegisterEffect(e5)
end
function c511001146.eqval(ec)
	return ec:IsLevelBelow(4) and ec:IsRace(RACE_DRAGON)
end
function c511001146.filter(c,tp)
	return c:IsLevelBelow(4) and c:IsRace(RACE_DRAGON) and c:CheckUniqueOnField(tp) and not c:IsForbc511001146den()
end
function c511001146.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and c511001146.filter(chkc,tp) end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,c511001146.filter,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil,tp)
...
function c511001146.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFaceup() and c:IsRelateToEffect(e) and tc and tc:IsRelateToEffect(e) and c511001146.filter(tc,tp) then
