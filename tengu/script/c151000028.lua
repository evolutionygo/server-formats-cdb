...
function c151000028.initial_effect(c)
	--Activate
	...
	e0:SetCode(c151000028)
	e0:SetValue(SUMMON_TYPE_NORMAL)
	c:RegisterEffect(e0)
	--summon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc151000028(c151000028,0))
	e2:SetProperty(EFFECT_FLAG_BOTH_SIDE)
	e2:SetCategory(CATEGORY_SUMMON)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_FZONE)
	e2:SetTarget(c151000028.target)
	e2:SetOperation(c151000028.operation)
	e2:SetLabelObject(e0)
	c:RegisterEffect(e2)
end
c151000028.af="a"
function c151000028.filter(c,se)
	if not c:IsSummonableCard() then return false end
	local mi,ma=c:GetTributeRequirement()
	return mi>0 and (c:IsSummonable(false,se) or c:IsMSetable(false,se))
end
function c151000028.get_targets(se,tp)
	local g=Duel.GetMatchingGroup(c151000028.filter,tp,LOCATION_HAND,0,nil,se)
	local minct=5
	local maxct=0
	local tc=g:GetFirst()
...
function c151000028.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local se=e:GetLabelObject()
	if chk==0 then
		local mi,ma=c151000028.get_targets(se,tp)
		if mi==5 then return false end
		return Duel.CheckLPCost(tp,mi*1000)
	end
	local mi,ma=c151000028.get_targets(se,tp)
	local ac=0
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringc151000028(c151000028,3))
...
function c151000028.sfilter(c,se,ct)
	if not c:IsSummonableCard() then return false end
	local mi,ma=c:GetTributeRequirement()
	return (mi==ct or ma==ct) and (c:IsSummonable(false,se) or c:IsMSetable(false,se))
end
function c151000028.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	local ct=e:GetLabel()
	local se=e:GetLabelObject()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SUMMON)
	local g=Duel.SelectMatchingCard(tp,c151000028.sfilter,tp,LOCATION_HAND,0,1,1,nil,se,ct)
	local tc=g:GetFirst()
	if tc then
