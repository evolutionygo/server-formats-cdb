...
function c53839837.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc53839837(c53839837,4))
	...
	e3:SetDescription(aux.Stringc53839837(c53839837,0))
...
function c53839837.spr(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetTurnPlayer()==tp and Duel.GetCurrentPhase()==PHASE_STANDBY then
		...
		c:RegisterFlagEffect(c53839837,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY+RESET_SELF_TURN,0,2)
	else
		e:SetLabel(0)
		c:RegisterFlagEffect(c53839837,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY+RESET_SELF_TURN,0,1)
	end
end
function c53839837.spcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return e:GetLabelObject():GetLabel()~=Duel.GetTurnCount() and tp==Duel.GetTurnPlayer() and c:GetFlagEffect(c53839837)>0
end
function c53839837.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local c=e:GetHandler()
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
	c:ResetFlagEffect(c53839837)
end
...
function c53839837.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringc53839837(c53839837,5))
	local op=Duel.SelectOption(tp,aux.Stringc53839837(c53839837,1),aux.Stringc53839837(c53839837,2),aux.Stringc53839837(c53839837,3))
	e:SetLabel(op)
