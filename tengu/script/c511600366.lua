...
function c511600366.initial_effect(c)
	--pendulum summon
	Pendulum.AddProcedure(c,false)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCode2N(c,true,true,aux.FilterBoolFunctionEx(Card.IsAttribute,ATTRIBUTE_DARK),3)
	--Venemy Counter
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_FIELD)
	e1:SetCode(EVENT_LEAVE_FIELD)
	e1:SetRange(LOCATION_MZONE)
	e1:SetOperation(c511600366.acop)
	c:RegisterEffect(e1)
	--atk down
	...
	e2:SetCondition(c511600366.adcon)
	e2:SetTarget(aux.NOT(aux.TargetBoolFunction(Card.IsStarvingVenemy)))
	e2:SetValue(c511600366.adval)
	c:RegisterEffect(e2)
	--atk up
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc511600366(c511600366,0))
	e3:SetCategory(CATEGORY_ATKCHANGE)
	e3:SetType(EFFECT_TYPE_TRIGGER_F+EFFECT_TYPE_SINGLE)
	e3:SetCode(EVENT_PRE_DAMAGE_CALCULATE)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c511600366.atkcon)
	e3:SetOperation(c511600366.atkop)
	c:RegisterEffect(e3)
	--negate
	...
	e4:SetCountLimit(1)
	e4:SetTarget(c511600366.distg)
	e4:SetOperation(c511600366.disop)
	c:RegisterEffect(e4)
end
function c511600366.acop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
	for tc in aux.Next(g) do
		tc:AddCounter(0x1149,#eg,true)
	end
end
function c511600366.adcon(e)
	return Duel.GetCurrentPhase()==PHASE_DAMAGE_CAL
end
function c511600366.adval(e,c)
	return Duel.GetCounter(e:GetHandlerPlayer(),LOCATION_ONFIELD,LOCATION_ONFIELD,0x1149)*-100
end
function c511600366.atkcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler()==Duel.GetAttacker() and Duel.GetCounter(tp,LOCATION_ONFIELD,LOCATION_ONFIELD,0x1149)>0
end
function c511600366.atkop(e,tp,eg,ep,ev,re,r,rp)
...
function c511600366.distg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c511600366.disop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
