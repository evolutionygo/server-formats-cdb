...
function c511010107.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511010107(c511010107,0))
	...
	e2:SetTarget(c511010107.distg)
	e2:SetOperation(c511010107.disop)
	...
	e3:SetCountLimit(1)
	e3:SetCondition(c511010107.discon)
	e3:SetCost(c511010107.discost)
	...
	aux.GlobalCheck(s,function()
		...
		ge1:SetOperation(function() Duel.RegisterFlagEffect(0,c511010107,RESET_PHASE|PHASE_BATTLE,0,1) end)
		Duel.RegisterEffect(ge1,0)
		--Keep track of which card was the last one that declared an attack
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_ATTACK_ANNOUNCE)
		ge2:SetOperation(c511010107.atkregop)
		Duel.RegisterEffect(ge2,0)
		c511010107.last_attacker=nil
	end)
end
c511010107.aux.xyz_number=107
c511010107.listed_series={SET_NUMBER}
function c511010107.atkregop(e,tp,eg,ep,ev,re,r,rp)
	local ac=eg:GetFirst()
	c511010107.last_attacker=ac
	ac:RegisterFlagEffect(c511010107,RESET_EVENT|RESETS_STANDARD|RESET_PHASE|PHASE_BATTLE,0,1)
end
function c511010107.discon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsBattlePhase() and Duel.IsTurnPlayer(tp) and not Duel.GetAttacker() and Duel.GetCurrentChain()==0
		and not Duel.HasFlagEffect(tp,c511010107+1) and not Duel.IsPhase(PHASE_BATTLE_START)
end
function c511010107.discost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:CheckRemoveOverlayCard(tp,1,REASON_COST) end
	c:RemoveOverlayCard(tp,1,1,REASON_COST)
	if Duel.IsTurnPlayer(tp) then
		Duel.RegisterFlagEffect(tp,c511010107+1,RESET_PHASE|PHASE_BATTLE,0,1)
		c:RegisterFlagEffect(c511010107+1,RESET_EVENT|RESETS_STANDARD|RESET_PHASE|PHASE_DAMAGE|PHASE_BATTLE,0,1)
		...
		e1:SetTarget(function(e,c) return c~=c511010107.last_attacker or not c:HasFlagEffect(c511010107+1) end)
...
function c511010107.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c511010107.disop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local exc=c:IsRelateToEffect(e) and c or nil
	local res=false
	local g=Duel.GetMatchingGroup(c511010107.disfilter,tp,LOCATION_MZONE,LOCATION_MZONE,exc)
	for tc in g:Iter() do
		--Negate its effects
		tc:NegateEffects(c)
	...
	local resolved_eff_ct=Duel.GetFlagEffect(0,c511010107)
	...
	if res and atk_ct==1 and c:CanAttack() and c511010107.last_attacker==c and c:HasFlagEffect(c511010107) then
		...
		e5:SetOperation(c511010107.endbpop)
		...
		e6:SetCondition(function(e) return not c:HasFlagEffect(c511010107) end)
...
function c511010107.endbpop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
