...
function c511001822.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc511001822(c511001822,0))
...
function c511001822.tgfilter(c,e,tp)
	local rk=c:GetRank()
	local pg=aux.GetMustBeMaterialGroup(tp,Group.FromCards(c),tp,nil,nil,REASON_XYZ)
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsFaceup() and (rk>0 or c:IsStatus(STATUS_NO_LEVEL))
		and Duel.IsExistingMatchingCard(c511001822.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,rk+1)
end
function c511001822.spfilter(c,e,tp,mc,rk)
...
function c511001822.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c511001822.tgfilter(chkc,e,tp) end
	if chk==0 then return Duel.IsExistingTarget(c511001822.tgfilter,tp,LOCATION_MZONE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,c511001822.tgfilter,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
...
function c511001822.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c511001822.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetRank()+1)
	local sc=g:GetFirst()
	if sc then
		sc:SetMaterial(tc)
		Duel.Overlay(sc,tc)
...
function c511001822.batval(e,re,c)
