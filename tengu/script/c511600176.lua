...
function c511600176.activate(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) or Duel.GetFlagEffect(tp,c511600176)>0 then return end
	local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(c511600176.thfilter),tp,LOCATION_GRAVE,0,nil)
	if #g>0 and Duel.SelectYesNo(tp,aux.Stringc511600176(35371948,0)) then
		Duel.RegisterFlagEffect(tp,c511600176,RESET_PHASE+PHASE_END,0,1)
