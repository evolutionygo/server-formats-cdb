...
function c511600271.initial_effect(c)
	--reverse atk/def
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e1:SetCode(EVENT_ADJUST)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(c511600271.condition)
	e1:SetOperation(c511600271.operation)
	c:RegisterEffect(e1)
	--destroy
	...
	e2:SetCondition(c511600271.descon)
	e2:SetTarget(c511600271.destg)
	e2:SetOperation(c511600271.desop)
	c:RegisterEffect(e2)
	if not zeroDay then zeroDay={} end
	if not zeroDayReff then zeroDayReff={} end
end
c511600271.listed_series={0x23b}
function c511600271.filter(c,ec)
	return c:IsLinkMonster() and not c:IsSetCard(0x23b) and c:GetLinkedGroup():IsContains(ec)
end
function c511600271.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c511600271.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,e:GetHandler())
end
function c511600271.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local lg=Duel.GetMatchingGroup(c511600271.filter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,e:GetHandler())
...
function c511600271.cfilter(c,tp)
...
function c511600271.descon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c511600271.cfilter,1,nil,tp)
end
function c511600271.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,e:GetHandler(),1,0,0)
end
function c511600271.desop(e,tp,eg,ep,ev,re,r,rp)
