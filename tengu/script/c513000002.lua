...
function c513000002.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if a and a:IsControler(1-tp) and a:IsFaceup() and a:IsLocation(LOCATION_MZONE) then
		...
		if a:GetFlagEffect(c513000002)==0 then
			a:RegisterFlagEffect(c513000002,RESET_EVENT+RESETS_STANDARD,0,1)
		end
	end
end
function c513000002.atktg(e,c)
	return c:GetFlagEffect(c513000002)~=0 and e:GetLabelObject():IsContains(c)
end
