...
function c511002874.initial_effect(c)
	...
	e4:SetValue(c511002874.efilter1)
	c:RegisterEffect(e4)
	--Dinosaur and Winged Beast monsters are unaffected by your opponent's Trap effects
	local e5=e2:Clone()
	e5:SetCode(EFFECT_IMMUNE_EFFECT)
	e5:SetValue(c511002874.efilter2)
	c:RegisterEffect(e5)
	--Change that monster to Defense Position
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringc511002874(c511002874,0))
	e6:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e6:SetCode(EVENT_BE_BATTLE_TARGET)
	e6:SetRange(LOCATION_FZONE)
	e6:SetCondition(c511002874.cbcon)
	e6:SetOperation(c511002874.cbop)
	c:RegisterEffect(e6)
end
function c511002874.efilter1(e,re,rp)
	return re:IsTrapEffect() and e:GetHandlerPlayer()~=rp
end
function c511002874.efilter2(e,te)
