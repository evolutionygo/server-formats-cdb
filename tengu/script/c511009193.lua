...
function c511009193.rdcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return ep~=tp and c==Duel.GetAttacker() and Duel.GetAttackTarget()==nil and c:GetFlagEffect(c511009193)<c:GetOverlayCount() 
end
function c511009193.rdop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	c:RegisterFlagEffect(c511009193,RESET_EVENT+RESETS_STANDARD_DISABLE+RESET_PHASE+PHASE_BATTLE,0,1)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
...
function c511009193.dircon(e)
	local c=e:GetHandler()
	return c:GetFlagEffect(c511009193)<c:GetOverlayCount() 
end
function c511009193.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.SelectYesNo(tp,aux.Stringc511009193(49678559,1)) then
...
function c511009193.damop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:CheckRemoveOverlayCard(tp,1,REASON_EFFECT) and Duel.SelectYesNo(tp,aux.Stringc511009193(48739166,1)) then
