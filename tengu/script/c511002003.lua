...
function c511002003.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511002003(95100065,0))
	...
	e3:SetDescription(aux.Stringc511002003(95100065,1))
	...
	c:RegisterEffect(e3)
	Duel.AddCustomActivityCounter(c511002003,ACTIVITY_SPSUMMON,c511002003.counterfilter)
end
...
function c511002003.damcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetCustomActivityCount(c511002003,tp,ACTIVITY_SPSUMMON)==0 end
...
function c511002003.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_PZONE) and chkc:IsControler(tp) and c511002003.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511002003.filter,tp,LOCATION_PZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c511002003.filter,tp,LOCATION_PZONE,0,1,1,nil)
