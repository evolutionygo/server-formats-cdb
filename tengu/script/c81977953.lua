...
function c81977953.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc81977953(81977953,0))
...
function c81977953.rescon(sg,e,tp,mg)
	return aux.ChkfMMZ(1)(sg,e,tp,mg) and sg:IsExists(c81977953.atchk1,1,nil,sg)
end
function c81977953.atchk1(c,sg)
...
function c81977953.spcon(e,c)
	...
	local rg1=Duel.GetMatchingGroup(c81977953.spfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil,ATTRIBUTE_EARTH)
	local rg2=Duel.GetMatchingGroup(c81977953.spfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil,ATTRIBUTE_WIND)
	local rg=rg1:Clone()
	rg:Merge(rg2)
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3 and #rg1>0 and #rg2>1 
		and aux.SelectUnselectGroup(rg,e,tp,3,3,c81977953.rescon,0)
end
function c81977953.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local rg=Duel.GetMatchingGroup(c81977953.spfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil,ATTRIBUTE_EARTH+ATTRIBUTE_WIND)
	local g=aux.SelectUnselectGroup(rg,e,tp,3,3,c81977953.rescon,1,tp,HINTMSG_REMOVE,nil,nil,true)
	if #g>0 then
		g:KeepAlive()
		e:SetLabelObject(g)
		return true
