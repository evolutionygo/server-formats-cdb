...
function c511000246.initial_effect(c)
	...
	e2:SetTarget(c511000246.sptg)
	e2:SetOperation(c511000246.spop)
	c:RegisterEffect(e2)
	--1000 Damage
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc511000246(c511000246,0))
	...
	e5:SetDescription(aux.Stringc511000246(c511000246,1))
...
function c511000246.exfilter(c)
	return c511000246.spfilter(c) or (c:IsFacedown() and c:IsSpell() and c:IsAbleToGraveAsCost())
end
function c511000246.spcon(e,c)
	...
	if Duel.IsPlayerAffectedByEffect(tp,54828837) then
		g=Duel.GetMatchingGroup(c511000246.exfilter,tp,LOCATION_ONFIELD,0,nil)
	else
		g=Duel.GetMatchingGroup(c511000246.spfilter,tp,LOCATION_ONFIELD,0,nil)
	end
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3 and #g>2 and aux.SelectUnselectGroup(g,e,tp,3,3,aux.ChkfMMZ(1),0)
end
function c511000246.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local g=nil
	if Duel.IsPlayerAffectedByEffect(tp,54828837) then
		g=Duel.GetMatchingGroup(c511000246.exfilter,tp,LOCATION_ONFIELD,0,nil)
	else
...
function c511000246.nodmgop(e,tp,eg,ep,ev,re,r,rp)
	...
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc511000246(c511000246,1))
