...
c511002214.listed_names={96470883} --Queen Angel of Roses
c511002214.listed_series={0x218} -- "Angel" archetype
...
function c511002214.spcon(e,c)
	...
	local rg=Duel.GetMatchingGroup(c511002214.spfilter,tp,LOCATION_MZONE,0,nil,ft)
	return ft>-1 and #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,1,nil,0)
end
function c511002214.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local rg=Duel.GetMatchingGroup(c511002214.spfilter,tp,LOCATION_MZONE,0,nil,ft)
...
function c511002214.atkfilter(c,tp)
...
function c511002214.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c511002214.atkfilter,1,nil,tp) and not eg:IsContains(e:GetHandler()) end
end
function c511002214.atkop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) or c:IsFacedown() then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELECT)
	local g=eg:FilterSelect(tp,c511002214.atkfilter,1,1,nil,tp)
	local tc=g:GetFirst()
	if tc then
