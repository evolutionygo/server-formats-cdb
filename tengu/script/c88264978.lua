...
function c88264978.initial_effect(c)
	...
	e1:SetCountLimit(1,c88264978,EFFECT_COUNT_CODE_OATH)
	e1:SetCondition(c88264978.spcon1)
	e1:SetTarget(c88264978.sptg1)
	e1:SetOperation(c88264978.spop1)
	c:RegisterEffect(e1)
	--Special summon a dragon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc88264978(c88264978,0))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetCountLimit(1,{c88264978,1})
	e2:SetRange(LOCATION_MZONE)
	e2:SetTarget(c88264978.sptg)
	e2:SetOperation(c88264978.spop)
	c:RegisterEffect(e2)
end
c88264978.listed_names={c88264978}
...
function c88264978.spcon1(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local rg=Duel.GetMatchingGroup(c88264978.spfilter,tp,LOCATION_MZONE,0,nil,ft)
	return ft>-1 and #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,1,nil,0)
end
function c88264978.sptg1(e,tp,eg,ep,ev,re,r,rp,c)
	local c=e:GetHandler()
	local g=nil
	local rg=Duel.GetMatchingGroup(c88264978.spfilter,tp,LOCATION_MZONE,0,nil,Duel.GetLocationCount(tp,LOCATION_MZONE))
...
function c88264978.spop1(e,tp,eg,ep,ev,re,r,rp,c)
...
function c88264978.filter(c,e,tp)
	return c:IsRace(RACE_DRAGON) and not c:IsCode(c88264978) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
