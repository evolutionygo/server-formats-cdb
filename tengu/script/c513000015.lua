...
function c513000015.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc513000015(c513000015,0))
	...
	e2:SetDescription(aux.Stringc513000015(c513000015,1))
	...
	e3:SetDescription(aux.Stringc513000015(c513000015,2))
	...
	e3:SetTarget(c513000015.rmtg)
	e3:SetOperation(c513000015.rmop)
	c:RegisterEffect(e3)
	--Return this card to the field
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringc513000015(c513000015,3))
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e4:SetCode(EVENT_PHASE+PHASE_END)
	e4:SetRange(LOCATION_REMOVED)
	e4:SetCountLimit(1)
	e4:SetCondition(function(e,tp) return e:GetHandler():HasFlagEffect(c513000015) and Duel.IsTurnPlayer(1-tp) end)
	...
	e5:SetDescription(aux.Stringc513000015(c513000015,4))
	e5:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e5:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e5:SetCode(EVENT_BATTLE_DESTROYED)
	e5:SetTarget(c513000015.sptg)
	e5:SetOperation(c513000015.spop)
	c:RegisterEffect(e5)
end
c513000015.synchro_tuner_required=1
c513000015.synchro_nt_required=1
...
function c513000015.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToRemove() end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,c,1,tp,0)
end
function c513000015.rmop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.Remove(c,POS_FACEUP,REASON_EFFECT|REASON_TEMPORARY)>0
		...
		c:RegisterFlagEffect(c513000015,RESET_EVENT|RESETS_STANDARD,0,0)
	end
end
function c513000015.spfilter(c,e,tp,sync)
...
function c513000015.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>=ct
		and mg:FilterCount(aux.NecroValleyFilter(c513000015.spfilter),nil,e,tp,c)==ct then
