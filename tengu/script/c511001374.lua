...
function c511001374.initial_effect(c)
	...
	aux.AddXyzProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_HERALDIC_BEAST),4,2)
	...
	e2:SetDescription(aux.Stringc511001374(c511001374,0))
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_MZONE)
	e2:SetHintTiming(TIMING_BATTLE_PHASE,TIMING_BATTLE_PHASE)
	e2:SetCost(aux.dxmcostgen(1,1,nil))
	e2:SetTarget(c511001374.efftg)
	e2:SetOperation(c511001374.effop)
	c:RegisterEffect(e2,false,REGISTER_FLAG_DETACH_XMAT)
end
c511001374.listed_series={SET_NUMBER}
c511001374.listed_names={CARD_UNKNOWN}
c511001374.aux.xyz_number=8
function c511001374.efffilter(c)
	return c:IsNegatableMonster() and c:IsAttackPos()
end
function c511001374.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local b1=true
	local b2=Duel.IsExistingMatchingCard(c511001374.efffilter,tp,0,LOCATION_MZONE,1,nil)
	...
	local op=Duel.SelectEffect(tp,{b1,aux.Stringc511001374(c511001374,1)},{b2,aux.Stringc511001374(c511001374,2)},{b3,aux.Stringc511001374(c511001374,3)})
	e:SetLabel(op)
	if op==2 then
...
function c511001374.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==1 then
		...
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringc511001374(c511001374,1))
	...
	elseif op==2 then
		...
		local sc=Duel.SelectMatchingCard(tp,c511001374.efffilter,tp,0,LOCATION_MZONE,1,1,nil):GetFirst()
