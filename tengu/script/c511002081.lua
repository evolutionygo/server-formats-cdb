...
function c511002081.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc511002081(95100147,0))
...
function c511002081.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc511002081(99946920,1))
...
function c511002081.regfilter(c)
	return c.synchro_type and c:IsType(TYPE_SYNCHRO) and c:GetFlagEffect(c511002081+1)==0
end
function c511002081.synchk(e,tp,eg,ep,ev,re,r,rp)
	local sg=Duel.GetMatchingGroup(c511002081.regfilter,tp,0xff,0xff,nil)
	local tc=sg:GetFirst()
	while tc do
		tc:RegisterFlagEffect(c511002081+1,0,0,0)
		...
		if tc.synchro_type==1 then
			...
			e1:SetCondition(Synchro.Condition(f1,min1,max1,f2,min2,max2,sub1,sub2,req1,req2,c511002081.reqm(reqm)))
			e1:SetTarget(Synchro.Target(f1,min1,max1,f2,min2,max2,sub1,sub2,req1,req2,c511002081.reqm(reqm)))
		...
		elseif tc.synchro_type==2 then
			...
			e1:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_IGNORE_IMMUNE)
			e1:SetRange(LOCATION_GRAVE)
			e1:SetCondition(Synchro.Condition(table.unpack(t),c511002081.reqm()))
			e1:SetTarget(Synchro.Target(table.unpack(t),c511002081.reqm()))
			e1:SetOperation(Synchro.Operation)
			e1:SetValue(SUMMON_TYPE_SYNCHRO)
			tc:RegisterEffect(e1)
...
function c511002081.reqm(reqm)
	return function(g,sc,tp)
				return g:IsExists(Card.IsHasEffect,1,nil,c511002081) and (not reqm or reqm(g,sc,tp))
			end
end