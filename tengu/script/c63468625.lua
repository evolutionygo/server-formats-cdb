...
function c63468625.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc63468625(c63468625,0))
	...
	e4:SetDescription(aux.Stringc63468625(c63468625,1))
...
function c63468625.spcon(e,c)
	...
	local rg=Duel.GetMatchingGroup(c63468625.spfilter,tp,LOCATION_HAND,0,e:GetHandler())
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and #rg>2 and aux.SelectUnselectGroup(rg,e,tp,3,3,nil,0)
end
function c63468625.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local c=e:GetHandler()
	local g=nil
	local rg=Duel.GetMatchingGroup(c63468625.spfilter,tp,LOCATION_HAND,0,c)
...
function c63468625.equipop(c,e,tp,tc)
	...
	if not c:EquipByEffectAndLimitRegister(e,tp,tc,c63468625) then return end
	local e2=Effect.CreateEffect(c)
...
function c63468625.dcfilter(c)
	return c:GetFlagEffect(c63468625)~=0 and c:IsAbleToGraveAsCost()
end
