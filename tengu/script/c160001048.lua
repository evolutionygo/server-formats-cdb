--突風
--Gust (Rush Duel)
function c160001048.initial_effect(c)
	--Destroy 1 spell/trap on the field
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_TO_GRAVE)
	e1:SetCondition(c160001048.condition)
	e1:SetTarget(c160001048.target)
	e1:SetOperation(c160001048.activate)
	c:RegisterEffect(e1)
end
function c160001048.filter(c,tp)
...
function c160001048.condition(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and eg:IsExists(c160001048.filter,1,nil,tp)
end
function c160001048.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c160001048.activate(e,tp,eg,ep,ev,re,r,rp)
	--Effect
