...
function c100000110.initial_effect(c)
	...
	e2:SetCondition(c100000110.spcon)
	e2:SetTarget(c100000110.sptg)
	...
	e3:SetOperation(c100000110.atkup)
	c:RegisterEffect(e3)
	--Unaffected
	...
	e4:SetValue(c100000110.efilter)
	c:RegisterEffect(e4)
	aux.GlobalCheck(s,function()
		...
		e0:SetCountLimit(1)
		e0:SetOperation(c100000110.endop)
...
function c100000110.endop(e,tp,c)
	...
	local rc=g:GetFirst()
	while rc do
		if rc:GetFlagEffect(c100000110)==0 then
			...
			rc:RegisterFlagEffect(c100000110,RESET_EVENT+RESETS_STANDARD,0,1)
		end
		rc=g:GetNext()
	end
end
...
function c100000110.ctop(e,tp,c)
	...
	local ct=e:GetLabel()
	if c:GetFlagEffect(c100000110)~=0 and ct==8 then 
		c:RegisterFlagEffect(c100000110+1,RESET_EVENT+RESETS_STANDARD,0,1)
	else
		e:SetLabel(ct+1)
	end
end
function c100000110.filter(c,tp)
	return c:IsFaceup() and c:IsLevel(1) and c:IsType(TYPE_NORMAL) and c:GetFlagEffect(c100000110+1)~=0
end
function c100000110.spcon(e,c)
	if c==nil then return true end
	return Duel.CheckReleaseGroup(c:GetControler(),c100000110.filter,1,false,1,true,c,c:GetControler(),nil,false,nil)
end
function c100000110.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.SelectReleaseGroup(tp,c100000110.filter,1,1,false,true,true,c,nil,nil,false,nil)
	if g then
		g:KeepAlive()
		e:SetLabelObject(g)
	return true
