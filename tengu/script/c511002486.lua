...
function c511002486.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc511002486(79491903,0))
...
function c511002486.unregop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetAttacker()==c or (Duel.GetAttackTarget() and Duel.GetAttackTarget()==c) and c:GetFlagEffect(c511002486)>0 then
		c:ResetFlagEffect(c511002486)
	end
end
function c511002486.regop(e,tp,eg,ep,ev,re,r,rp)
	if ep~=tp then
		e:GetHandler():RegisterFlagEffect(c511002486,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_BATTLE,0,1)
	end
end
function c511002486.damcon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsHasEffect(c511002486)
end
