...
function c75402014.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc75402014(c75402014,0))
	...
	e2:SetDescription(aux.Stringc75402014(c75402014,0))
	e2:SetCategory(CATEGORY_EQUIP)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetCode(EVENT_CHAINING)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1,0,EFFECT_COUNT_CODE_SINGLE)
	e2:SetCondition(c75402014.eqcon)
	e2:SetTarget(c75402014.eqtg)
	e2:SetOperation(c75402014.eqop)
	c:RegisterEffect(e2)
	--Negate opponent's cards, up to number of "ZW -" monsters equipped
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc75402014(c75402014,1))
	e2:SetCategory(CATEGORY_DISABLE)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1)
	e2:SetCost(c75402014.descost)
	e2:SetTarget(c75402014.destg)
	e2:SetOperation(c75402014.desop)
	c:RegisterEffect(e2,false,REGISTER_FLAG_DETACH_XMAT)
end
c75402014.listed_series={0x107e}
c75402014.aux.xyz_number=39
function c75402014.filter(c,tc,tp)
	if not (c:IsSetCard(0x107e) and not c:IsForbc75402014den()) then return false end
	local effs={c:GetCardEffect(c75402014)}
...
function c75402014.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c75402014.filter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,e:GetHandler(),tp) end
...
function c75402014.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c75402014.filter,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,c,tp)
	local tc=g:GetFirst()
	if tc then
		local eff=tc:GetCardEffect(c75402014)
...
function c75402014.descost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c75402014.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsNegatable() and chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_ONFIELD) end
	if chk==0 then return e:GetHandler():GetEquipGroup():IsExists(c75402014.cfilter,1,nil)
...
function c75402014.desop(e,tp,eg,ep,ev,re,r,rp)
