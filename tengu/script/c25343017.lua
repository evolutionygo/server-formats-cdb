...
function c25343017.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc25343017(c25343017,0))
	...
	e3:SetDescription(aux.Stringc25343017(c25343017,1))
	...
	e4:SetDescription(aux.Stringc25343017(c25343017,2))
...
function c25343017.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 and c:IsRelateToEffect(e) then
		tc:RegisterFlagEffect(c25343017,RESET_EVENT+RESETS_STANDARD,0,0)
		e:GetLabelObject():SetLabel(1)
		if c:GetFlagEffect(c25343017)==0 then
			c:RegisterFlagEffect(c25343017,RESET_EVENT|RESETS_STANDARD&~(RESET_LEAVE|RESET_TEMP_REMOVE|RESET_TOGRAVE|RESET_TURN_SET),0,0)
...
function c25343017.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	if act==1 and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD)
		and e:GetHandler():GetFlagEffect(c25343017) then return true
	else rg:Clear() return false end
end
function c25343017.spfilter(c,e,tp)
	return c:GetFlagEffect(c25343017)~=0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
