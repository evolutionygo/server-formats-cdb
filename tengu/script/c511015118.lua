...
function c511015118.initial_effect(c)
	...
	e2:SetOperation(c511015118.replacetg)
	c:RegisterEffect(e2)
	...
	e3:SetTarget(c511015118.replacetg)
	c:RegisterEffect(e3)
	...
	e4:SetTarget(c511015118.replacetg)
	e4:SetValue(c511015118.repval)
	c:RegisterEffect(e4)
	...
	e6:SetCountLimit(1)
	e6:SetCondition(c511015118.atkcon)
	e6:SetOperation(c511015118.atkop)
	c:RegisterEffect(e6)
	...
	e7:SetCondition(function(e) return e:GetHandler():HasFlagEffect(c511015118) end)
	...
	e8:SetCondition(function(e) return e:GetHandler():HasFlagEffect(c511015118) end)
	e8:SetOperation(c511015118.leaveop)
	c:RegisterEffect(e8)
end
c511015118.listed_series={SET_NUMBER}
c511015118.listed_names={1426715} --"Phantom Token"
c511015118.aux.xyz_number=48
function c511015118.mvfilter(c)
	return c:HasFlagEffect(c511015118) and c:GetSequence()<5
end
function c511015118.replacetg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ft==0 then return end
	if Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) then ft=math.min(ft,1) end
	if chk==0 then return not c:HasFlagEffect(c511015118) and not c:IsReason(REASON_REPLACE) and c:GetReasonPlayer()==1-tp 
	...
	c:RegisterFlagEffect(c511015118,RESET_EVENT|RESETS_STANDARD_DISABLE|RESET_PHASE|PHASE_END,0,1)
	...
	for i=1,ft do
		...
		token:RegisterFlagEffect(c511015118,RESET_EVENT|RESETS_STANDARD_DISABLE|RESET_PHASE|PHASE_END,0,1)
		tg:AddCard(token)
		...
		e2:SetOperation(c511015118.damop)
		token:RegisterEffect(e2)
	end
	Duel.SpecialSummonComplete()
	Duel.BreakEffect()
	local sg=Duel.GetMatchingGroup(c511015118.mvfilter,tp,LOCATION_MZONE,0,nil)
...
function c511015118.repval(e,c)
...
function c511015118.atkcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_MZONE,0,1,nil,1426715)
		and not e:GetHandler():HasFlagEffect(c511015118)
end
function c511015118.atkop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c511015118.leaveop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
