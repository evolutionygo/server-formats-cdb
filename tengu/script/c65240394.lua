...
function c65240394.initial_effect(c)
	--to attack
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e1:SetCode(EVENT_DAMAGE_STEP_END)
	e1:SetOperation(c65240394.posop)
	c:RegisterEffect(e1)
	...
	aux.GlobalCheck(s,function()
		...
		ge:SetOperation(c65240394.negop)
		Duel.RegisterEffect(ge,0)
	end)
end
function c65240394.posop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c==Duel.GetAttackTarget() and c:IsDefensePos() and c:IsRelateToBattle() then
		Duel.ChangePosition(c,POS_FACEUP_ATTACK)
	end
end
function c65240394.negop(e,tp,eg,ep,ev,re,r,rp)
