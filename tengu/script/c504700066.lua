...
function c504700066.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCondition(c504700066.condition)
	c:RegisterEffect(e1)
	--
	...
	e2:SetTarget(c504700066.sumlimit)
	e2:SetLabel(0)
	c:RegisterEffect(e2)
	...
	e3:SetOperation(c504700066.sumop)
	c:RegisterEffect(e3)
	s[c]={}
	s[c][0]=0
	s[c][1]=0
end
function c504700066.condition(e,tp,eg,ep,ev,re,r,rp)
	s[e:GetHandler()][tp]=0
	return Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)<=2
		and Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)<=2
end
function c504700066.sumlimit(e,c,sp,st)
	return s[e:GetHandler()][sp]>=2
end
function c504700066.sumop(e,tp,eg,ep,ev,re,r,rp)
