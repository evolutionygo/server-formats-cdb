...
function c83121692.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCode2(c,true,true,21844576,20721928,79979666)
	--spsummon condition
	...
	--set target
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringc83121692(c83121692,0))
...
function c83121692.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c==tc then
		tc:RegisterFlagEffect(c83121692,RESET_EVENT+RESETS_STANDARD,0,0)
	else
		c:SetCardTarget(tc)
	end
	if c:GetFlagEffect(c83121692+1)==0 then
		c:RegisterFlagEffect(c83121692+1,RESET_EVENT+RESETS_STANDARD,0,0)
		local e1=Effect.CreateEffect(c)
...
function c83121692.indtg(e,c)
	if c==e:GetHandler() then
		return c:GetFlagEffect(c83121692)~=0
	else
