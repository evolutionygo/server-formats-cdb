...
function c76052811.initial_effect(c)
	...
	e3:SetDescription(aux.Stringc76052811(c76052811,0))
...
function c76052811.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) then
		c:RegisterFlagEffect(c76052811,RESET_EVENT+RESETS_STANDARD,0,0)
	end
end
function c76052811.hdcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()~=tp and e:GetHandler():GetFlagEffect(c76052811)~=0
end
function c76052811.hdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	e:GetHandler():ResetFlagEffect(c76052811)
	Duel.SetOperationInfo(0,CATEGORY_HANDES,0,0,1-tp,1)
end
function c76052811.hdop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsRelateToEffect(e) then
		e:GetHandler():RegisterFlagEffect(c76052811,RESET_EVENT+RESETS_STANDARD,0,0)
