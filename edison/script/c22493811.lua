...
c22493811.listed_names={22493812}
...
function c22493811.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local check=not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) and Duel.IsPlayerCanSpecialSummonMonster(tp,c22493811+1,0,TYPES_TOKEN,500,1200,4,RACE_INSECT,ATTRIBUTE_EARTH)
		local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
...
function c22493811.activate(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) and Duel.GetLocationCount(tp,LOCATION_MZONE)>1
		and Duel.IsPlayerCanSpecialSummonMonster(tp,c22493811+1,0,TYPES_TOKEN,500,1200,4,RACE_INSECT,ATTRIBUTE_EARTH) then
		for i=1,2 do
			local token=Duel.CreateToken(tp,c22493811+1)
