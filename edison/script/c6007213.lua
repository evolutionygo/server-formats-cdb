...
function c6007213.initial_effect(c)
	...
	e4:SetDescription(aux.Stringc6007213(c6007213,0))
...
function c6007213.spfilter(c,tp)
	return c:IsFaceup() and c:IsTrap() and c:IsAbleToGraveAsCost()
end
function c6007213.exfilter(c,tp)
...
function c6007213.spcon(e,c)
	...
	if Duel.IsPlayerAffectedByEffect(tp,16317140) then
		g=Duel.GetMatchingGroup(c6007213.exfilter,tp,LOCATION_ONFIELD,0,nil)
	else
		g=Duel.GetMatchingGroup(c6007213.spfilter,tp,LOCATION_ONFIELD,0,nil)
	end
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3 and #g>2 and aux.SelectUnselectGroup(g,e,tp,3,3,aux.ChkfMMZ(1),0)
end
function c6007213.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local g=nil
	if Duel.IsPlayerAffectedByEffect(tp,16317140) then
		g=Duel.GetMatchingGroup(c6007213.exfilter,tp,LOCATION_ONFIELD,0,nil)
	else
