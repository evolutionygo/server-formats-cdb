...
function c31550470.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc31550470(c31550470,0))
	...
	e3:SetOperation(c31550470.mondesop)
	...
	e4:SetCondition(c31550470.selfdescon)
	e4:SetOperation(function(e) Duel.Destroy(e:GetHandler(),REASON_EFFECT) end)
	c:RegisterEffect(e4)
end
function c31550470.spfilter(c,e,tp)
...
function c31550470.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c31550470.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c31550470.spfilter,tp,LOCATION_REMOVED,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c31550470.spfilter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp)
...
function c31550470.mondesop(e,tp,eg,ep,ev,re,r,rp)
...
function c31550470.selfdescon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetHandler():GetFirstCardTarget()
