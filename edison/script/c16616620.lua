...
function c16616620.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc16616620(c16616620,0))
...
c16616620.listed_series={SET_CHRYSALIS,SET_NEO_SPACIAN}
function c16616620.spfilter(c,e,tp,g)
...
function c16616620.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return #g>0 and g:FilterCount(Card.IsAbleToGrave,nil)==#g and Duel.GetMZoneCount(tp,g)>0
		and Duel.IsExistingMatchingCard(c16616620.spfilter,tp,LOCATION_HAND|LOCATION_DECK,0,1,nil,e,tp,g) end
...
function c16616620.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg=Duel.SelectMatchingCard(tp,c16616620.spfilter,tp,LOCATION_HAND|LOCATION_DECK,0,1,1,nil,e,tp,og)
	if #sg>0 then
