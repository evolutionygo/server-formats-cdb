...
function c52101615.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc52101615(c52101615,0))
...
function c52101615.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		local mg=Duel.GetMatchingGroup(c52101615.fil2,tp,LOCATION_MZONE,0,nil,nil,MATERIAL_FUSION)
		local mg2=Duel.GetMatchingGroup(c52101615.fil,tp,0,LOCATION_MZONE,nil,tp)
		Fusion.CheckAdditional=c52101615.fcheck(mg2)
		local res=Duel.IsExistingMatchingCard(c52101615.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg+mg2,c,chkf)
		Fusion.CheckAdditional=nil
		return res
	end
	local mg=Duel.GetMatchingGroup(c52101615.fil2,tp,LOCATION_MZONE,0,nil)
	local mg2=Duel.GetMatchingGroup(c52101615.fil,tp,0,LOCATION_MZONE,nil,tp)
	Fusion.CheckAdditional=c52101615.fcheck(mg2)
