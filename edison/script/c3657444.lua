...
function c3657444.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc3657444(c3657444,0))
	e1:SetCategory(CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_BE_BATTLE_TARGET)
	e1:SetCost(aux.SelfBanishCost)
	e1:SetTarget(c3657444.drawtg1)
	e1:SetOperation(c3657444.drawop1)
	c:RegisterEffect(e1)
	--Draw 2 cards
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringc3657444(c3657444,1))
	e2:SetCategory(CATEGORY_REMOVE+CATEGORY_DRAW)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_MZONE)
	e2:SetTarget(c3657444.drawtg2)
	e2:SetOperation(c3657444.drawop2)
	c:RegisterEffect(e2)
	--Banish itself and 1 card from the hand, then place 1 card of the top of the Deck
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc3657444(c3657444,2))
	e3:SetCategory(CATEGORY_REMOVE+CATEGORY_TODECK)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_MZONE)
	e3:SetTarget(c3657444.tdtg)
	e3:SetOperation(c3657444.tdop)
	c:RegisterEffect(e3)
end
function c3657444.drawtg1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) end
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c3657444.drawop1(e,tp,eg,ep,ev,re,r,rp)
	Duel.Draw(tp,1,REASON_EFFECT)
	Duel.SkipPhase(1-tp,PHASE_BATTLE,RESET_PHASE|PHASE_BATTLE_STEP,1)
end
function c3657444.drawtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c3657444.drawop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c3657444.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c3657444.tdop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
