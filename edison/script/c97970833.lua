...
function c97970833.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc97970833(c97970833,0))
...
c97970833.listed_names={34487429} --"Ancient City - Rainbow Ruins"
function c97970833.actfilter(c,tp)
...
function c97970833.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sc=Duel.SelectMatchingCard(tp,c97970833.actfilter,tp,LOCATION_DECK,0,1,1,nil,tp):GetFirst()
	if not sc then return end
	local fc=Duel.GetFieldCard(1-tp,LOCATION_FZONE,0)
	local can_draw=fc and fc:IsFaceup() and Duel.IsPlayerCanDraw(1-tp,1)
	if Duel.ActivateFieldSpell(sc,e,tp,eg,ep,ev,re,r,rp) and can_draw and Duel.SelectYesNo(1-tp,aux.Stringc97970833(c97970833,1)) then
		Duel.BreakEffect()
