...
function c511002998.disfilter(c,im0,im1,re)
...
function c511002998.discheck(ev,category,re,im0,im1,targets)
	...
	if tg and #tg>0 then
		if targets and targets:IsContains(re:GetHandler()) then
			return tg:IsExists(c511002998.disfilter,1,nil,im0,im1,re)
		else
			return tg:IsExists(c511002998.disfilter,1,re:GetHandler(),im0,im1,re)
		end
	end
	return false
end
function c511002998.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not res and c511002998.discheck(ev,CATEGORY_TOEXTRA,re,im0,im1,targets) then res=true end
