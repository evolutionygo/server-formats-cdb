...
function c72258771.initial_effect(c)
	...
	local e2=aux.AddNormalSummonProcedure(c,true,true,1,1,SUMMON_TYPE_TRIBUTE,aux.Stringc72258771(c72258771,0),c72258771.otfilter)
	--pierce
...
function c72258771.mtop(e,tp,eg,ep,ev,re,r,rp)
	...
	local select=1
	if #g1>0 then
		select=Duel.SelectOption(tp,aux.Stringc72258771(c72258771,0),aux.Stringc72258771(c72258771,1))
	else
		select=Duel.SelectOption(tp,aux.Stringc72258771(c72258771,1))+1
	end
	if select==0 then
