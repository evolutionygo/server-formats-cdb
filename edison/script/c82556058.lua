...
function c82556058.initial_effect(c)
	...
	e1:SetDescription(aux.Stringc82556058(c82556058,0))
	...
	e2:SetDescription(aux.Stringc82556058(c82556058,1))
	e2:SetCategory(CATEGORY_ATKCHANGE)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1)
	e2:SetTarget(c82556058.atktg)
	e2:SetOperation(c82556058.atkop)
	c:RegisterEffect(e2)
	--Destroy this card during the End Phase
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringc82556058(c82556058,2))
	e3:SetCategory(CATEGORY_DESTROY)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e3:SetCode(EVENT_PHASE+PHASE_END)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCountLimit(1)
	e3:SetCondition(function(e) return e:GetHandler():HasFlagEffect(c82556058) end)
	e3:SetTarget(c82556058.selfdestg)
	e3:SetOperation(c82556058.selfdesop)
	c:RegisterEffect(e3)
end
c82556058.listed_names={TOKEN_ENGINE}
...
function c82556058.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	e:GetHandler():RegisterFlagEffect(c82556058,RESETS_STANDARD_PHASE_END,EFFECT_FLAG_OATH,1)
...
function c82556058.selfdestg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,e:GetHandler(),1,0,0)
end
function c82556058.selfdesop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
