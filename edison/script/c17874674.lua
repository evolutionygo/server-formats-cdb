...
function c17874674.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc17874674(c17874674,0))
...
function c17874674.target1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if Duel.GetTurnPlayer()~=tp and Duel.GetCurrentPhase()==PHASE_STANDBY
		...
		e:GetHandler():RegisterFlagEffect(c17874674,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	else
		e:SetProperty(0)
	end
end
...
function c17874674.target2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsNegatable() end
	if chk==0 then return e:GetHandler():GetFlagEffect(c17874674)==0 end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,Card.IsNegatable,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
	e:GetHandler():RegisterFlagEffect(c17874674,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
end
function c17874674.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetFlagEffect(c17874674)==0 or not c:IsRelateToEffect(e) then return end
...
function c17874674.costop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.CheckLPCost(tp,500) and Duel.SelectYesNo(tp,aux.Stringc17874674(c17874674,1)) then
