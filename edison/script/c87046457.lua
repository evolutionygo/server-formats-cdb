...
function c87046457.initial_effect(c)
	...
	e2:SetDescription(aux.Stringc87046457(c87046457,0))
...
function c87046457.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetFlagEffect(c87046457)==0 then
		c:RegisterFlagEffect(c87046457,RESET_EVENT+RESETS_STANDARD_DISABLE,0,1)
		atkg:Clear()
	end
	local g=eg:Filter(c87046457.filter,nil,e,tp)
	local tc=g:GetFirst()
	for tc in aux.Next(g) do
		tc:RegisterFlagEffect(c87046457,RESET_EVENT+RESETS_STANDARD,0,1)
		atkg:AddCard(tc)
	end
end
function c87046457.atkcon(e)
	return e:GetHandler():GetFlagEffect(c87046457)~=0
end
function c87046457.atktg(e,c)
	return e:GetLabelObject():IsContains(c) and c:GetFlagEffect(c87046457)~=0
end