...
function c49587034.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Remove(card,POS_FACEDOWN,REASON_EFFECT)>0 and e:IsHasType(EFFECT_TYPE_ACTIVATE) then
		...
		card:RegisterFlagEffect(c49587034,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		local descnum=tp==c:GetOwner() and 0 or 1
		local e3=Effect.CreateEffect(c)
		e3:SetType(EFFECT_TYPE_SINGLE)
		e3:SetDescription(aux.Stringc49587034(c49587034,descnum))
...
function c49587034.thcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsTurnPlayer(tp) and e:GetHandler():GetFlagEffect(c49587034)==0
end
